...
const { Channels, Servers, Users, Members, Podcasts } = require('../db/repositories');
...
async function requireChannelAdmin(req, res, next) {
  ...
  if (!isServerOwner && !isSiteAdmin) {
    ...
    if (!member) return res.status(403).json({ error: 'You do not have permission to manage this channel' });
  }
...
async function getPodcastSettings(channelId) {
...
router.get('/:channelId/rss', asyncHandler(async (req, res) => {
  const { channelId } = req.params;
  const channel  = await Channels.findById(channelId);
...
router.get('/:channelId/feed.json', asyncHandler(async (req, res) => {
  const { channelId } = req.params;
  const channel  = await Channels.findById(channelId);
  if (!channel) return res.status(404).json({ error: 'Channel not found' });
...
// ── Embed Player HTML ─────────────────────────────────────────
router.get('/embed/:episodeId', asyncHandler(async (req, res) => {
  const ep = await Podcasts.findEpisodeOne({ _id: req.params.episodeId, published: true });
...
// ── Bölüm Listesi ─────────────────────────────────────────────
router.get('/:channelId/episodes', asyncHandler(async (req, res) => {
  const { channelId } = req.params;
...
router.post('/:channelId/episodes', authMiddleware, requireChannelAdmin, asyncHandler(async (req, res) => {
...
// ── Bölüm Sil ─────────────────────────────────────────────────
router.delete('/:channelId/episodes/:episodeId', authMiddleware, requireChannelAdmin, asyncHandler(async (req, res) => {
  const { episodeId } = req.params;
  const ep = await Podcasts.findEpisodeOne({ _id: episodeId, channelId: req.params.channelId });
...
// ── Podcast Ayarları ──────────────────────────────────────────
router.patch('/:channelId/settings', authMiddleware, requireChannelAdmin, asyncHandler(async (req, res) => {
  const { channelId } = req.params;
...
router.post('/:channelId/record/start', authMiddleware, requireChannelAdmin, asyncHandler(async (req, res) => {
  const { channelId } = req.params;
...
router.post('/:channelId/record/stop', authMiddleware, requireChannelAdmin, asyncHandler(async (req, res) => {
  const _u = castAuthed(req).user;
  const { channelId } = req.params;
  ...
  if (activeRecordings.has(channelId)) {
    ...
    fs.writeFileSync(outputPath, Buffer.from(req.body.audioFile, 'base64'));
  } else {
...
router.get('/:channelId/record/status', authMiddleware, requireChannelAdmin, asyncHandler(async (req, res) => {
  const rec = activeRecordings.get(req.params.channelId);
