...
describe('WebAuthn Routes', () => {
  ...
  describe('POST /api/webauthn/login/begin', () => {
    ...
    it('session cache\'e challenge kaydedilmeli', async () => {
      ...
      const sessionKey = `webauthn:auth:${res.body.challenge}`;
      expect(mockCacheStore.get(sessionKey)).toEqual(
        expect.objectContaining({ challenge: res.body.challenge })
      );
    });
  });
  ...
  describe('Security', () => {
    ...
    it('login: replay attack (signCount düşük) → 401', async () => {
      ...
      const challenge = b64uEncode(crypto.randomBytes(32));
      mockCacheStore.set(`webauthn:auth:${challenge}`, {
        challenge,
        userId: 'test-user-id',
