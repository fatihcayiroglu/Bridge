// server/tests/federation-social.test.ts
...
beforeEach(async () => {
  await mockDb.users.update({ _id: USER2_ID }, { $set: { apPublicKey: undefined, apPrivateKey: undefined } });
  jest.clearAllMocks();
...
describe('POST /api/federation/follow', () => {
  ...
  it('409 — aynı aktörü zaten takip ediyorsa çakışma döner', async () => {
    ...
    // Temizle
    await mockDb.apOutgoingFollows.remove({ _id: 'existing-follow' });
  });
...
describe('GET /api/federation/following', () => {
  ...
  it('200 — takip edilenleri döner (doğru format)', async () => {
    ...
    await mockDb.apOutgoingFollows.remove({ _id: 'out-follow-1' });
  });
});
...
describe('GET /api/federation/profile', () => {
  ...
  it('isFollowing true döner — kullanıcı bu aktörü takip ediyorsa', async () => {
    ...
    await mockDb.apOutgoingFollows.remove({ _id: 'profile-follow' });
  });
