...
const { registerVoiceHandlers, leaveVoice, voiceRooms } = require('../socket/handlers/voice');
...
describe('voice:join', () => {
  it('odaya katılır, socket odaya eklenir', () => {
    const user   = makeUser();
    const socket = makeSocket('sock-1');
    const io     = makeIo();
    registerVoiceHandlers(socket, io, user);

    socket._trigger('voice:join', { channelId: 'ch-1', serverId: 'sv-1' });
  ...
  it('mevcut peer listesini yeni katılana gönderir', () => {
    ...
    socket1._trigger('voice:join', { channelId: 'ch-2', serverId: 'sv-1' });
    ...
    socket2._trigger('voice:join', { channelId: 'ch-2', serverId: 'sv-1' });
  ...
  it('odaya katılan herkese voice:peer-joined yayınlar', () => {
    const user1   = makeUser();
    const socket1 = makeSocket('sock-1');
    const io      = makeIo();
    registerVoiceHandlers(socket1, io, user1);
    socket1._trigger('voice:join', { channelId: 'ch-3', serverId: 'sv-1' });

    const user2   = makeUser({ displayName: 'NewPeer' });
    const socket2 = makeSocket('sock-2');
    registerVoiceHandlers(socket2, io, user2);
    socket2._trigger('voice:join', { channelId: 'ch-3', serverId: 'sv-1' });
  ...
  it('kapasite doluysa voice:full gönderir ve odaya eklemez', () => {
    const io = makeIo();
    const MAX = 10;
    ...
    for (let i = 0; i < MAX; i++) {
      ...
      s._trigger('voice:join', { channelId: 'ch-full', serverId: 'sv-1' });
    }
    ...
    lateSocket._trigger('voice:join', { channelId: 'ch-full', serverId: 'sv-1' });
  ...
  it('aktif müzik varsa yeni katılana music:play gönderir', () => {
    ...
    const io     = makeIo();
    registerVoiceHandlers(socket, io, user);
    socket._trigger('voice:join', { channelId: 'ch-music', serverId: 'sv-1' });
...
describe('voice:leave', () => {
  it('odadan ayrılır, voiceRooms güncellenir', () => {
    const user   = makeUser();
    const socket = makeSocket('sock-leave');
    const io     = makeIo();
    registerVoiceHandlers(socket, io, user);
    socket._trigger('voice:join',  { channelId: 'ch-leave', serverId: 'sv-1' });
    socket._trigger('voice:leave', { channelId: 'ch-leave', serverId: 'sv-1' });
  ...
  it('olmayan odadan leave çağrısı hata fırlatmaz', () => {
    const user   = makeUser();
    const socket = makeSocket('sock-safe-leave');
    const io     = makeIo();
    registerVoiceHandlers(socket, io, user);
    // Hiç join yapmadan leave — sessizce geçmeli
    expect(() => socket._trigger('voice:leave', { channelId: 'nonexistent', serverId: 'sv-1' })).not.toThrow();
  });
});
...
describe('leaveVoice() yardımcısı', () => {
  it('doğrudan çağrılınca da odayı temizler', () => {
    const user   = makeUser();
    const socket = makeSocket('sock-lv');
    const io     = makeIo();
    registerVoiceHandlers(socket, io, user);
    socket._trigger('voice:join', { channelId: 'ch-lv', serverId: 'sv-1' });

    leaveVoice(socket, 'ch-lv', 'sv-1', io);
...
describe('WebRTC sinyal iletimi', () => {
  ...
  it('webrtc:offer hedef socket\'e iletilir', () => {
    const { io, ioEmitted, s1 } = setup();
    s1._trigger('webrtc:offer', { targetSocketId: 'sock-rtc-2', offer: { sdp: 'test' }, channelId: 'ch-rtc' });
  ...
  it('webrtc:answer hedef socket\'e iletilir', () => {
    const { ioEmitted, s1 } = setup();
    s1._trigger('webrtc:answer', { targetSocketId: 'sock-rtc-2', answer: { sdp: 'answer-sdp' } });
  ...
  it('webrtc:ice-candidate hedef socket\'e iletilir', () => {
    const { ioEmitted, s1 } = setup();
    s1._trigger('webrtc:ice-candidate', { targetSocketId: 'sock-rtc-2', candidate: { candidate: 'ice-cand' } });
...
describe('voice:state-update', () => {
  it('mute/deafen/screenshare durumu odaya yayınlanır', () => {
    const io       = makeIo();
    const emitted  = [];
    const user     = makeUser();
    ...
    registerVoiceHandlers(socket, io, user);
    socket._trigger('voice:join', { channelId: 'ch-state', serverId: 'sv-1' });
    socket._trigger('voice:state-update', { channelId: 'ch-state', muted: true, deafened: false, screensharing: true, video: false });
...
describe('voice:activity', () => {
  it('konuşma durumu odaya yayınlanır', () => {
    const io      = makeIo();
    const emitted = [];
    const user    = makeUser();
    ...
    registerVoiceHandlers(socket, io, user);
    socket._trigger('voice:join', { channelId: 'ch-act', serverId: 'sv-1' });
    socket._trigger('voice:activity', { channelId: 'ch-act', speaking: true });
...
describe('voice:e2e-key', () => {
  it('şifreli anahtar hedef kullanıcının socket\'ine iletilir', () => {
    const io       = makeIo();
    const ioEmitted = [];
    io.to = (target) => ({
      emit(event, data) { ioEmitted.push({ event, data, _target: target }); },
    ...
    senderSocket._trigger('voice:join',   { channelId: 'ch-e2e', serverId: 'sv-1' });
    receiverSocket._trigger('voice:join', { channelId: 'ch-e2e', serverId: 'sv-1' });

    ioEmitted.length = 0; // join event'lerini temizle

    senderSocket._trigger('voice:e2e-key', {
      channelId:    'ch-e2e',
  ...
  it('odada olmayan kullanıcıya key iletmeye çalışmak sessizce geçer', () => {
    const io       = makeIo();
    const ioEmitted = [];
    ...
    registerVoiceHandlers(socket, io, user);
    socket._trigger('voice:join', { channelId: 'ch-e2e-alone', serverId: 'sv-1' });
    ioEmitted.length = 0;

    expect(() => {
      socket._trigger('voice:e2e-key', {
