// server/tests/federation-peers-inbox.test.js
// Kapsanan boşluklar:
...
// httpSignature — ping için federation imzasını mock'la
jest.mock('../lib/httpSignature', () => ({
  verifyHttpSignature:     jest.fn().mockReturnValue(true),
  verifyFederationRequest: jest.fn().mockReturnValue(true),
}));
...
const { deliverApActivity } = require('../routes/federation/delivery');
const inboxHandlers = require('../routes/federation/inbox-handlers');
...
describe('POST /api/federation/ping', () => {
  it('401 — federation imzası geçersizse reddeder', async () => {
    ...
    verifyFederationRequest.mockReturnValueOnce(false);
