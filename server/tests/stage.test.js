...
describe('stage:join', () => {
  it('socket odaya katılır ve mevcut state alır', () => {
    const user   = makeUser();
    const socket = makeSocket('s-join-1');
    const io     = makeIo();
    registerStageHandlers(socket, io, user);

    socket._trigger('stage:join', { channelId: 'ch-s1', serverId: 'sv-1' });
  ...
  it('channelId yoksa işlem yapılmaz', () => {
    const user   = makeUser();
    const socket = makeSocket('s-join-noop');
    const io     = makeIo();
    registerStageHandlers(socket, io, user);

    expect(() => socket._trigger('stage:join', {})).not.toThrow();
...
describe('stage:setRole', () => {
  it('kullanıcı speaker olarak eklenir', () => {
    const user   = makeUser({ displayName: 'Host' });
    const socket = makeSocket('s-role-1');
    const io     = makeIo();
    registerStageHandlers(socket, io, user);
    socket._trigger('stage:join',    { channelId: 'ch-role', serverId: 'sv-1' });
    socket._trigger('stage:setRole', { channelId: 'ch-role', role: 'speaker' });
  ...
  it('kullanıcı listener olarak eklenir', () => {
    const user   = makeUser({ displayName: 'Audience' });
    const socket = makeSocket('s-role-2');
    const io     = makeIo();
    registerStageHandlers(socket, io, user);
    socket._trigger('stage:join',    { channelId: 'ch-role-l', serverId: 'sv-1' });
    socket._trigger('stage:setRole', { channelId: 'ch-role-l', role: 'listener' });
  ...
  it('rol değiştirilince eski listeden kaldırılır', () => {
    const user   = makeUser();
    const socket = makeSocket('s-role-switch');
    const io     = makeIo();
    registerStageHandlers(socket, io, user);
    socket._trigger('stage:join',    { channelId: 'ch-switch', serverId: 'sv-1' });
    socket._trigger('stage:setRole', { channelId: 'ch-switch', role: 'speaker'  });
    socket._trigger('stage:setRole', { channelId: 'ch-switch', role: 'listener' });
  ...
  it('geçersiz rol sessizce reddedilir', () => {
    const user   = makeUser();
    const socket = makeSocket('s-role-bad');
    const io     = makeIo();
    registerStageHandlers(socket, io, user);
    socket._trigger('stage:join',    { channelId: 'ch-badrole', serverId: 'sv-1' });
    socket._trigger('stage:setRole', { channelId: 'ch-badrole', role: 'moderator' });
  ...
  it('speaker muted:true olarak başlar', () => {
    const user   = makeUser();
    const socket = makeSocket('s-muted-start');
    const io     = makeIo();
    registerStageHandlers(socket, io, user);
    socket._trigger('stage:join',    { channelId: 'ch-muted', serverId: 'sv-1' });
    socket._trigger('stage:setRole', { channelId: 'ch-muted', role: 'speaker'  });

    const room = stageRooms.get('ch-muted');
    expect(room.speakers[0].muted).toBe(true);
  });

  it('stage:userJoined ve stage:state odaya emit edilir', () => {
    const user   = makeUser();
    const socket = makeSocket('s-joined-emit');
    const io     = makeIo();
    registerStageHandlers(socket, io, user);
    socket._trigger('stage:join',    { channelId: 'ch-emit', serverId: 'sv-1' });
    io._emitted.length = 0;
    socket._trigger('stage:setRole', { channelId: 'ch-emit', role: 'speaker' });
...
describe('stage:updateMute', () => {
  it('speaker\'ın mute durumu güncellenir ve odaya yayınlanır', () => {
    const user   = makeUser();
    const socket = makeSocket('s-mute-upd');
    const io     = makeIo();
    registerStageHandlers(socket, io, user);
    socket._trigger('stage:join',        { channelId: 'ch-mupd', serverId: 'sv-1' });
    socket._trigger('stage:setRole',     { channelId: 'ch-mupd', role: 'speaker'  });
    io._emitted.length = 0;
    socket._trigger('stage:updateMute',  { channelId: 'ch-mupd', muted: false });
  ...
  it('odada olmayan channel\'da updateMute hata fırlatmaz', () => {
    const user   = makeUser();
    const socket = makeSocket('s-mute-noop');
    const io     = makeIo();
    registerStageHandlers(socket, io, user);
    expect(() => socket._trigger('stage:updateMute', { channelId: 'nonexistent', muted: true })).not.toThrow();
  });

  it('listener updateMute gönderirse speaker listesi değişmez', () => {
    ...
    hSocket._trigger('stage:join',    { channelId: 'ch-listmute', serverId: 'sv-1' });
    hSocket._trigger('stage:setRole', { channelId: 'ch-listmute', role: 'speaker' });

    const lSocket = makeSocket('s-listener');
    registerStageHandlers(lSocket, io, listener);
    lSocket._trigger('stage:join',        { channelId: 'ch-listmute', serverId: 'sv-1' });
    lSocket._trigger('stage:setRole',     { channelId: 'ch-listmute', role: 'listener' });
    io._emitted.length = 0;
    lSocket._trigger('stage:updateMute',  { channelId: 'ch-listmute', muted: false });
...
describe('stage:handRaise', () => {
  it('el kaldırma durumu odaya yayınlanır', () => {
    const user   = makeUser();
    const socket = makeSocket('s-hand');
    const io     = makeIo();
    registerStageHandlers(socket, io, user);
    socket._trigger('stage:join',      { channelId: 'ch-hand', serverId: 'sv-1' });
    socket._trigger('stage:setRole',   { channelId: 'ch-hand', role: 'listener' });
    io._emitted.length = 0;
    socket._trigger('stage:handRaise', { channelId: 'ch-hand', raised: true });
  ...
  it('el indirme çalışır', () => {
    const user   = makeUser();
    const socket = makeSocket('s-hand-down');
    const io     = makeIo();
    registerStageHandlers(socket, io, user);
    socket._trigger('stage:join',      { channelId: 'ch-hd', serverId: 'sv-1' });
    socket._trigger('stage:setRole',   { channelId: 'ch-hd', role: 'listener' });
    socket._trigger('stage:handRaise', { channelId: 'ch-hd', raised: true  });
    socket._trigger('stage:handRaise', { channelId: 'ch-hd', raised: false });
...
describe('stage:promote', () => {
  ...
  it('host listener\'ı speaker\'a yükseltebilir', async () => {
    const { io, host, hSock, listener } = setupRoom('ch-promote');
    io._emitted.length = 0;
  ...
  it('host olmayan kullanıcı promote edemez', async () => {
    const { io, listener, lSock } = setupRoom('ch-promote-deny');
    io._emitted.length = 0;
  ...
  it('odada olmayan listener promote edilemez', async () => {
    const { io, hSock } = setupRoom('ch-promote-missing');
    io._emitted.length = 0;
  ...
  it('yükseltilen kullanıcı muted ve handRaised:false başlar', async () => {
    const { hSock, listener } = setupRoom('ch-promote-state');
...
describe('stage:leave', () => {
  it('kullanıcı ayrılınca odadan kaldırılır', () => {
    const user   = makeUser();
    const socket = makeSocket('s-leave');
    const io     = makeIo();
    registerStageHandlers(socket, io, user);
    socket._trigger('stage:join',    { channelId: 'ch-leave', serverId: 'sv-1' });
    socket._trigger('stage:setRole', { channelId: 'ch-leave', role: 'speaker' });
    io._emitted.length = 0;
    socket._trigger('stage:leave',   { channelId: 'ch-leave' });
  ...
  it('son kullanıcı ayrılınca oda Map\'ten silinir', () => {
    const user   = makeUser();
    const socket = makeSocket('s-last');
    const io     = makeIo();
    registerStageHandlers(socket, io, user);
    socket._trigger('stage:join',    { channelId: 'ch-last', serverId: 'sv-1' });
    socket._trigger('stage:setRole', { channelId: 'ch-last', role: 'listener' });
    socket._trigger('stage:leave',   { channelId: 'ch-last' });
...
describe('disconnect — otomatik temizlik', () => {
  it('disconnect olunca tüm odalardan kaldırılır', () => {
    const user   = makeUser();
    ...
    ['ch-disc-1', 'ch-disc-2'].forEach(ch => {
      socket._trigger('stage:join',    { channelId: ch, serverId: 'sv-1' });
      socket._trigger('stage:setRole', { channelId: ch, role: 'speaker' });
    });
  ...
  it('disconnect sonrası stage:userLeft emit edilir', () => {
    const user   = makeUser();
    const socket = makeSocket('s-disc-evt');
    const io     = makeIo();
    registerStageHandlers(socket, io, user);
    socket._trigger('stage:join',    { channelId: 'ch-disc-evt', serverId: 'sv-1' });
    socket._trigger('stage:setRole', { channelId: 'ch-disc-evt', role: 'listener' });
    io._emitted.length = 0;
