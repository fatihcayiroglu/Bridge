...
import secureChunkUploadRouter              from '../routes/secureChunkUpload';
...
import { router as boostsRouter }           from '../routes/boosts';
import { router as spotifyOAuthRouter }     from '../routes/spotify-oauth';
import { router as announcementRouter, setIo as setAnnouncementIo } from '../routes/announcement';
import serverEventsRouter                   from '../routes/serverEvents';
import notificationPrefsRouter              from '../routes/notificationPrefs';
import serverMemberProfileRouter            from '../routes/serverMemberProfile';
...
import stickerPacksRouter                   from '../routes/sticker-packs';
...
export function setupRoutes(app: Application): void {
  ...
  // Hardened resumable upload endpoint. Kept separate until clients migrate.
  mountApi('/upload-secure', secureChunkUploadRouter);
  ...
  mountApi('/bots/marketplace', botMarketplaceRouter);
  ...
  mountApi('/servers', boostsRouter);
  mountApi('/oauth', spotifyOAuthRouter);
  mountApi('/channels', announcementRouter);
  mountApi('/servers', serverEventsRouter);
  mountApi('/notification-prefs', notificationPrefsRouter);
  ...
  mountApi('/servers', stickerPacksRouter);
  app.get('/metrics', metricsEndpoint);

  app.get('/.well-known/webfinger', (req: Request, res: Response, next: NextFunction) => {
  ...
  app.get('/.well-known/nodeinfo', async (_req: Request, res: Response) => {
    ...
    res.json({ links: [{ rel: 'http://nodeinfo.diaspora.software/ns/schema/2.1', href: `${instanceUrl}/nodeinfo/2.1` }] });
  });
  app.get('/nodeinfo/2.1', async (_req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res.json({ version: '2.1', software: { name: 'bridge', version: (pkg as { version: string }).version }, protocols: ['activitypub'], usage: { users: { total: userCount || 0, activeMonth: 0, activeHalfyear: 0 }, localPosts: postCount || 0 }, openRegistrations: !(process.env.DISABLE_REGISTRATION === 'true'), metadata: { nodeName: process.env.INSTANCE_NAME || 'Bridge Instance', nodeDescription: process.env.INSTANCE_DESC || 'A Bridge chat server' } });
