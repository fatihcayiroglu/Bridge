// plugins/welcome-bot/index.js — Bridge Plugin Örneği
'use strict';
...
async function setup(ctx) {
  ...
  ctx.hooks.on('member:joined', async ({ userId, serverId, displayName, username }) => {
    try {
      // Hedef kanalı bul
      const channels = await ctx.db.channels.find({ serverId, type: 'text' });
      const channel  = channels.find(c => c.name?.toLowerCase() === chanTarget)
                    || channels[0];
      if (!channel) return;
      ...
      // Socket üzerinden mesaj emit et (server index.js'deki io erişimi hook ile)
  ...
  ctx.registerRoute('GET', '/config', (req, res) => {
