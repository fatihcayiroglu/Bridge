...
(function () {
  ...
  async function showLocalNotification(title, body, data) {
    ...
    try {
      await LocalNotifications.schedule({ notifications: [{ title: title, body: body, id: Date.now(), extra: data || {}, smallIcon: 'ic_stat_bridge', iconColor: '#5865f2' }] });
    } catch (_) {}
  }

  // ── DEEP LINK ──────────────────────────────────────────────────
  // bridge://invite/<code>
  // bridge://channel/<serverId>/<channelId>
  // bridge://dm/<userId>
  // bridge://user/<userId>
  function handleDeepLink(url) {
    ...
    switch (action) {
      case 'invite':
        window.dispatchEvent(new CustomEvent('bridge:deeplink:invite', { detail: { code: segments[1] } }));
        break;
      case 'channel':
        window.dispatchEvent(new CustomEvent('bridge:navigate', { detail: { serverId: segments[1], channelId: segments[2] } }));
        break;
      case 'dm':
        window.dispatchEvent(new CustomEvent('bridge:deeplink:dm', { detail: { userId: segments[1] } }));
        break;
      case 'user':
        window.dispatchEvent(new CustomEvent('bridge:deeplink:user', { detail: { userId: segments[1] } }));
        break;
      default:
        console.warn('[Bridge Mobile] Bilinmeyen deep link:', action);
    }
  }
