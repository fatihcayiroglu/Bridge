// ⚠️  DEPRECATED — Sprint 106
// Bu dosya artık sw.ts'ten derleniyor. Doğrudan düzenleme yapmayın.
// Kaynak: client/sw.ts  |  Derleme: tsc --project tsconfig.sw.json
// Bu dosya geriye dönük uyumluluk için tutulmaktadır (eski tarayıcı fallback).
// ─────────────────────────────────────────────────────────────────────────────
...
// CACHE_VERSION: build.js tarafından üretilen asset-manifest.json'daki `version`
// alanından türetilir. Sabit değer yerine manifest'teki version kullanılır,
// böylece içerik değiştiğinde SW otomatik güncellenir.
const STATIC_CACHE_PREFIX = 'bridge-static';
const ALL_CACHES_PREFIX   = STATIC_CACHE_PREFIX;

// ── Install: asset-manifest.json'dan hash'li dosya listesini al ───────────
// build.js bu dosyayı client/dist/ altına yazar.
// Dev modunda manifest yoksa sabit fallback listesi kullanılır.
self.addEventListener('install', event => {
  event.waitUntil((async () => {
    let assets = ['/', '/css/style.css', '/css/tokens.css'];
    let version = 'dev';
    ...
    // version'dan türetilen cache ismi: içerik değişince eski cache silinir
    ...
    caches.match(event.request).then(cached => {
      ...
      return fetch(event.request).then(response => {
        if (response && response.status === 200 && response.type === 'basic') {
          ...
          caches.open(self.CURRENT_CACHE || STATIC_CACHE_PREFIX).then(cache => cache.put(event.request, clone));
        }
        broadcastNetworkStatus(true);
        return response;
      }).catch(async () => {
