  ...
  try {
    ...
    body.innerHTML = `
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        ...
        ${actor.url ? `<a href="${escHtml(actor.url)}" target="_blank" class="btn" style="flex:1;text-align:center;">Profili Görüntüle ↗</a>` : ''}
      </div>
    ...
    cont.innerHTML = items.map(msg => {
      ...
      return `<div style="border-bottom:1px solid var(--bg-4);padding:12px 0;">
        ...
        <div style="display:flex;gap:8px;margin-top:6px;">
          ${msg.apId ? `
            <button onclick="fedLikeNote('${escHtml(msg.apId)}')" class="btn" style="font-size:11px;padding:3px 8px;">❤️</button>
            <button onclick="fedAnnounceNote('${escHtml(msg.apId)}')" class="btn" style="font-size:11px;padding:3px 8px;">🔁</button>
          ` : ''}
        </div>
      </div>`;
    }).join('');
