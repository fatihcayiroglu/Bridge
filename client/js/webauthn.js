      ...
      <div id="passkey-item-${c.id}" style="display:flex;align-items:center;gap:10px;padding:10px;background:#1e1f22;border-radius:8px;margin-bottom:6px;">
        ...
        <div style="flex:1;min-width:0;">
          ...
          <div style="font-size:11px;color:var(--text-muted);">
            ...
            ${c.lastUsedAt ? ` · Son kullanım: ${formatDate(c.lastUsedAt)}` : ''}
          </div>
        </div>
        <div style="display:flex;gap:6px;">
          <button class="btn btn-secondary" style="padding:4px 8px;font-size:12px;"
            onclick="promptRenamePasskey('${c.id}', '${escHtml(c.name).replace(/'/g, "\\'")}')">✏️</button>
          <button class="btn" style="padding:4px 8px;font-size:12px;background:#e8432d;"
            onclick="confirmDeletePasskey('${c.id}', '${escHtml(c.name).replace(/'/g, "\\'")}')">🗑️</button>
        </div>
      </div>
    `).join('');
  } catch (err) {
...
function getDeviceIcon(deviceType = '') {
  if (/yubikey/i.test(deviceType)) return '🔐';
  if (/iphone|ios|face id|touch id/i.test(deviceType)) return '📱';
  if (/android/i.test(deviceType)) return '🤖';
  if (/windows|hello/i.test(deviceType)) return '💻';
  if (/platform/i.test(deviceType)) return '🖥️';
  return '🔑';
}
...
async function addNewPasskey() {
  ...
  try {
    ...
    if (btn) { btn.textContent = '🔑 Kayıt Yap'; btn.disabled = false; }
...
function injectPasskeyLoginButton() {
  ...
  btn.innerHTML = `<span style="font-size:18px;">🔑</span> Passkey ile Giriş Yap`;
  btn.onclick   = async () => {
...
export const getBridgeWebAuthn = () => window.BridgeWebAuthn;
