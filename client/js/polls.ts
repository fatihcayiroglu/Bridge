...
// â”€â”€ OPEN POLL CREATOR MODAL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function openPollCreator() {
  ...
  modal.innerHTML = `
    <div class="modal-card" style="max-width:480px;width:95%;">
      <h2 style="margin-bottom:4px;">ğŸ“Š Anket OluÅŸtur</h2>
      <p style="color:var(--text-muted);font-size:13px;margin-bottom:20px;">TÃ¼m Ã¼yeler oy verebilir â€” bedava âœ¦</p>
      ...
      <label class="settings-label">SeÃ§enekler <span style="color:var(--text-muted);font-size:12px;">(en az 2, en fazla 10)</span></label>
      <div id="poll-options-list" style="display:flex;flex-direction:column;gap:8px;margin-bottom:12px;">
        <div class="poll-opt-row"><input class="input poll-opt-input" placeholder="SeÃ§enek 1" maxlength="100" style="width:100%;"/></div>
        <div class="poll-opt-row"><input class="input poll-opt-input" placeholder="SeÃ§enek 2" maxlength="100" style="width:100%;"/></div>
      </div>
      <button class="btn btn-secondary" onclick="addPollOption()" style="margin-bottom:16px;font-size:13px;">+ SeÃ§enek Ekle</button>
      ...
      <div style="display:flex;gap:16px;margin-bottom:16px;flex-wrap:wrap;">
        ...
        <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;">
          ...
          <select id="poll-duration" class="input" style="padding:4px 8px;font-size:13px;">
            ...
            <option value="4320">3 gÃ¼n</option>
            <option value="10080">7 gÃ¼n</option>
          </select>
        </label>
      </div>

      <div class="modal-footer">
        <button class="btn btn-secondary" onclick="document.getElementById('poll-creator-modal').remove()">Ä°ptal</button>
        <button class="btn" onclick="submitPoll()">ğŸ“Š Anketi GÃ¶nder</button>
      </div>
    </div>`;
...
function addPollOption() {
  ...
  if (count >= 10) return toast('Maksimum 10 seÃ§enek', 'error');
  const row = document.createElement('div');
  row.className = 'poll-opt-row';
  row.style.display = 'flex';
  row.style.gap = '8px';
  row.innerHTML = `
    <input class="input poll-opt-input" placeholder="SeÃ§enek ${count + 1}" maxlength="100" style="flex:1;"/>
    <button class="btn btn-secondary" onclick="this.parentElement.remove()" style="padding:6px 10px;">âœ•</button>`;
...
async function submitPoll() {
  ...
  if (options.length < 2) return toast('En az 2 seÃ§enek gerekli', 'error');
  ...
  try {
    ...
    if (!r.ok) { const e = await r.json(); return toast(e.error || 'Anket oluÅŸturulamadÄ±', 'error'); }
    ...
    toast('Anket oluÅŸturuldu!', 'success');
  } catch (e) {
    console.error(e);
    toast('Anket oluÅŸturulamadÄ±', 'error');
  }
}

// â”€â”€ RENDER POLL INSIDE A MESSAGE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function renderPollBlock(poll, currentUserId) {
  ...
  if (poll.expiresAt && !closed) {
    ...
    timeLabel = `<span style="color:var(--text-muted);font-size:12px;">â± ${h}s ${m}d kaldÄ±</span>`;
  } else if (closed) {
    timeLabel = `<span style="color:var(--text-muted);font-size:12px;">ğŸ”’ Anket kapandÄ±</span>`;
  }
  ...
  const optionsHtml = poll.options.map(opt => {
    ...
    return `<div class="poll-option ${voted ? 'poll-voted' : ''} ${closed ? 'poll-closed' : ''}"
      ...
      <div class="poll-opt-content">
        <span class="poll-opt-text">${escHtml(opt.text)} ${voted ? 'âœ“' : ''}</span>
    ...
    <div class="poll-header">
      ...
      <div>
        ...
        <div style="display:flex;gap:8px;align-items:center;margin-top:2px;">
          ${totalVotes} oy â€¢ ${poll.multiSelect ? 'Ã‡oklu seÃ§im' : 'Tek seÃ§im'}
...
async function closePoll(pollId) {
  if (!confirm('Anketi kapatmak istediÄŸine emin misin?')) return;
  try {
    await apiFetch(`${API}/api/polls/${pollId}/close`, { method: 'POST' });
    toast('Anket kapatÄ±ldÄ±', 'success');
    ...
    toast('Anket kapatÄ±lamadÄ±', 'error');
  }
}
