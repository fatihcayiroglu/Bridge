  ...
  try {
    ...
    grid.innerHTML = _soundboardSounds.map(s => `
      <button class="sound-btn" onclick="playSound('${s._id}')" title="${escHtml(s.name)}">
        <span class="sound-emoji">${escHtml(s.emoji || '🔊')}</span>
...
function openSoundUpload() {
  ...
  modal.innerHTML = `
    <div class="modal-card" style="max-width:400px;width:95%;">
      <h2 style="margin-bottom:16px;">🎵 Ses Ekle</h2>
      ...
      <input id="sound-emoji-input" class="input" placeholder="🔊" maxlength="8"
             style="width:80px;margin-bottom:16px;font-size:20px;text-align:center;" value="🔊" />
      ...
      <div class="modal-footer">
        ...
        <button class="btn" onclick="uploadSound()">🎵 Yükle</button>
      </div>
    </div>`;
...
async function uploadSound() {
  ...
  const emoji = document.getElementById('sound-emoji-input')?.value.trim() || '🔊';
