// client/js/twoFactor.ts — 2FA Kurulum UI
// Sprint 33 FIX: window._twofa_url → modül-seviyesi _twofaUrl değişkeni.
// Hassas TOTP URL'si artık global window'a sızdırılmıyor.

let _twofaUrl: string | null = null;
  ...
  if (status.enabled) {
    modal.innerHTML = `
      <div class="modal-card" style="max-width:440px;width:95%;">
        <h2>ğŸ” İki Faktörlü Doğrulama</h2>
...
async function start2FASetup() {
  ...
  try {
    if (typeof QRCode !== 'undefined') {
      ...
      if (qrEl) {
        qrEl.innerHTML = `<a href="${data.otpauthUrl}" style="font-size:11px;color:var(--brand);">Authenticator'da Aç</a>`;
      }
    }
  } catch {}

  // Kodu kaydet — modül-seviyesi değişkende sakla (window'a sızdırma)
  _twofaUrl = data.otpauthUrl;
}
...
async function verify2FA() {
  ...
  if (modal) {
    ...
    modalCard.innerHTML = `
      ...
      <p style="color:#e8432d;font-weight:600;">âš ï¸ Yedek Kodlarını Kaydet!</p>
  ...
  toast('2FA aktif! Yedek kodlarını sakla ğŸ”', 'success');
}
  ...
  modal.innerHTML = `
    <div class="modal-card" style="max-width:440px;width:95%;">
      ...
      ${me.email ? `
        <div style="background:var(--bg-3);border-radius:8px;padding:12px;margin-bottom:16px;">
          ...
          <div style="color:${me.emailVerified?'#57f287':'#faa61a'};font-size:13px;margin-top:4px;">
            ${me.emailVerified ? '✅ Doğrulandı' : 'âš ï¸ Doğrulanmadı'}
          </div>
      ...
      <div class="modal-footer">
        ...
        <button class="btn" onclick="saveEmail()">ğŸ’¾ Kaydet & Doğrula</button>
      </div>
    </div>`;
  modal.onclick = (e) => { if (e.target === modal) modal.remove(); };
  document.body.appendChild(modal);
...
async function saveEmail() {
  ...
  toast(data.message || 'Doğrulama e-postası gönderildi ğŸ“§', 'success');
}
