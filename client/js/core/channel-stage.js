...
// Stage kanal UI — channel-permissions.js refactor'undan ayrıldı (v51'den aynen)

// ══════════════════════════════════════════════════════════════
// STAGE CHANNEL — değiştirilmedi (v51'den aynen)
// ══════════════════════════════════════════════════════════════
...
function toggleStageHand() {
  ...
  if (btn) btn.textContent = _stageHandRaised ? '✋ El İndir' : '✋ El Kaldır';
    ...
    case 'stage:handRaise': {
      ...
      if (u) {
        ...
        if (data.raised && data.userId !== me?.id && currentServer?.ownerId === me?.id) {
          toast(`✋ ${u.displayName} konuşmak istiyor`, 'info');
        }
      }
      break;
    }
