...
const ACTIVITY_LABELS = {
  ...
  listening: 'Dinliyor',
  watching:  'Ä°zliyor',
  streaming: 'YayÄ±n yapÄ±yor',
  coding:    'Kod yazÄ±yor',
  reading:   'Okuyor',
  custom:    'Ã–zel durum',
};

// â”€â”€ Aktivite formatla â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function formatActivity(activity) {
  ...
  return {
    ...
    fullText: detail ? `${name} â€” ${detail}` : name,
  };
}

// â”€â”€ Aktivite rozeti HTML â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Aktivite Ayar ModalÄ± â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function openActivityModal() {
  const existing = document.getElementById('activity-modal');
  if (existing) { existing.remove(); return; }

  const currentActivity = window.me?.activity;
  ...
  modal.innerHTML = `
    <div class="modal-card" style="max-width:420px;width:95%;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
        ...
        <button class="icon-btn" onclick="document.getElementById('activity-modal').remove()">âœ•</button>
      </div>
      ...
      <div id="activity-form" style="display:${currentActivity ? 'block' : 'none'};">
        <label class="settings-label">Ne yapÄ±yorsunuz?</label>
        <input id="activity-name" class="input" style="width:100%;margin-bottom:10px;"
               placeholder="Ã¶rn: Minecraft, Spotify, YouTube"
               value="${escHtml(currentActivity?.name || '')}" maxlength="64">

        <label class="settings-label">Detay (isteÄŸe baÄŸlÄ±)</label>
        <input id="activity-detail" class="input" style="width:100%;margin-bottom:16px;"
               placeholder="Ã¶rn: Creative mode, Lo-fi playlist"
               value="${escHtml(currentActivity?.detail || '')}" maxlength="128">

        <div class="modal-footer">
          <button class="btn btn-secondary" onclick="document.getElementById('activity-modal').remove()">Ä°ptal</button>
          <button class="btn" onclick="saveActivity()">âœ… Kaydet</button>
        </div>
      </div>

      ${!currentActivity ? `
        <p style="color:var(--text-muted);font-size:13px;text-align:center;">
          Bir aktivite tipi seÃ§in ğŸ‘†
        </p>` : ''}
    </div>`;
...
const ACTIVITY_TYPES_CLIENT = {
  ...
  watching:  { icon: 'ğŸ“º', label: 'Ä°zliyor' },
  streaming: { icon: 'ğŸ”´', label: 'YayÄ±n' },
  coding:    { icon: 'ğŸ’»', label: 'Kodluyor' },
  reading:   { icon: 'ğŸ“š', label: 'Okuyor' },
  custom:    { icon: 'âœï¸', label: 'Ã–zel' },
};
...
function selectActivityType(type) {
  ...
  if (nameInput) {
    const examples = {
      playing:   'Ã¶rn: Minecraft, Valorant, Steam',
      listening: 'Ã¶rn: Spotify, YouTube Music',
      watching:  'Ã¶rn: Netflix, YouTube',
      streaming: 'Ã¶rn: Twitch kanalÄ±m',
      coding:    'Ã¶rn: Bridge projesi, React app',
      reading:   'Ã¶rn: Dune, Medium makalesi',
      custom:    'Ne yapÄ±yorsunuz?',
    };
    nameInput.placeholder = examples[type] || 'Ne yapÄ±yorsunuz?';
    nameInput.focus();
  }
}

async function saveActivity() {
  const type   = _selectedActivityType || (window.me?.activity?.type) || 'custom';
  const name   = document.getElementById('activity-name')?.value.trim();
  const detail = document.getElementById('activity-detail')?.value.trim();

  if (!name) return toast('Ne yaptÄ±ÄŸÄ±nÄ±zÄ± yazÄ±n', 'error');
  ...
  try {
    ...
    if (window.me) window.me.activity = data.activity;
    document.getElementById('activity-modal')?.remove();
    toast(`${ACTIVITY_ICONS[type]} Aktivite gÃ¼ncellendi!`, 'success');
...
async function clearActivity() {
  try {
    ...
    if (window.me) window.me.activity = null;
    document.getElementById('activity-modal')?.remove();
...
function updateActivityDisplay() {
  // Kendi profilindeki aktivite badge'ini gÃ¼ncelle
...
// â”€â”€ Socket handler: baÅŸkalarÄ±nÄ±n aktivitesini al â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function handleUserActivity(data) {
  const { userId, activity } = data;

  // Ãœye listesinde gÃ¼ncelle
