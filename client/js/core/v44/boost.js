...
async function openBoostPanel(serverId) {
  let boost = { count: 0, boosters: [] };
  try {
  ...
  modal.innerHTML = `
    <div class="modal-card boost-card" style="max-width:500px;width:95%;max-height:88vh;overflow-y:auto;">
      ...
      <div class="boost-perks">
        <div style="font-weight:600;font-size:13px;margin-bottom:8px;">✨ Mevcut Ayrıcalıklar</div>
        ${tier.perks.map(p => `<div class="boost-perk-item">✅ ${p}</div>`).join('')}
        ${nextTier ? `
          <div style="font-weight:600;font-size:13px;margin:12px 0 8px;color:var(--text-muted);">🔒 ${nextTier.label} Ayrıcalıkları</div>
          ${nextTier.perks.filter(p => !tier.perks.includes(p)).map(p => `<div class="boost-perk-item locked">🔒 ${p}</div>`).join('')}
        ` : ''}
      </div>

      <button class="btn btn-primary boost-btn" onclick="sendBoost('${serverId}')">
        🚀 Bu Sunucuyu Boost'la
      </button>

      ${boost.boosters?.length ? `
        <div style="margin-top:16px;">
          <div style="font-weight:600;font-size:13px;margin-bottom:8px;">💜 Boost'çular (${boost.boosters.length})</div>
...
function injectBoostBar() {
  ...
  const sid = getCurrentServer()?._id;
  if (!sid) return;
  ...
  bar.innerHTML = `<span style="font-size:11px;color:var(--text-muted);">🚀</span><div class="boost-mini-fill" id="boost-mini-fill"></div>`;
