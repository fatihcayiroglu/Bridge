...
// Modül: Slow Mode UI — Kanal yöneticisi cooldown ayarı
'use strict';
...
const BridgeSlowMode = (() => {
  ...
  async function saveSlowMode(channelId, serverId, seconds) {
    ...
    toast(seconds > 0 ? `🐢 Yavaş mod ${seconds}s olarak ayarlandı` : '✅ Yavaş mod kapatıldı', 'success');
  }

  return { renderSlowModeBadge, startCooldown, saveSlowMode };
})();

// Kanal değiştiğinde slow mode badge'ini güncelle
...
if (typeof _origSwitchChannel === 'function') {
  window.switchChannel = async function(channelId) {
    ...
    if (getCurrentChannel()) BridgeSlowMode.renderSlowModeBadge(getCurrentChannel());
    return result;
  };
}

// Mesaj gönderilince slow mode cooldown başlat
...
if (typeof _origSendMsg === 'function') {
  window.sendMessage = async function() {
    ...
    const sm = getCurrentChannel()?.slowmode;
