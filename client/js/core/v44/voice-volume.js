...
// Modül: BridgeVoiceVolume — Kullanıcı başına ses seviyesi (sağ tıkla → slider)
'use strict';
...
const BridgeVoiceVolume = (() => {
  ...
  function attachGain(socketId, audioEl) {
    ...
    try {
      ...
      // createMediaElementSource: bir element'e iki kez bağlanamaz — sessizce atla
    }
  }
  ...
  function openVolumePanel(socketId, userId, displayName, anchorEl) {
    ...
    panel.innerHTML = `
      <div class="bvv-header">
        🔊 ${_esc(displayName)} Ses Seviyesi
        <button class="bvv-close" onclick="document.getElementById('bvv-panel')?.remove()">✕</button>
      </div>
      <div class="bvv-row">
      ...
      <div class="bvv-presets">
        <button onclick="_bvvSet(0)">🔇 Kapat</button>
    ...
    window._bvvSet = (val) => {
      document.getElementById('bvv-slider').value = val;
