...
// ModÃ¼l: Go Live (Ekran PaylaÅŸÄ±mÄ±) GeliÅŸtirmeleri
'use strict';

// Ä°zleyici listesi (voice channel peer'larÄ±ndan tÃ¼retilir)
const _goLiveViewers = new Set();

function updateGoLiveViewerCount(count) {
  const el = document.getElementById('ss-viewer-count');
  if (el) el.textContent = count > 0 ? `ğŸ‘ï¸ ${count} izleyici` : '';
}
...
function addGoLiveViewer(userId, displayName) {
  ...
  renderViewerList();
  toast(`${displayName || 'Birisi'} ekranÄ±nÄ± izlemeye baÅŸladÄ± ğŸ‘ï¸`, 'info');
}
...
function renderViewerList() {
  ...
  el.innerHTML = [..._goLiveViewers].map(uid => {
    ...
    return `<div style="display:flex;align-items:center;gap:6px;padding:4px 0;font-size:12px;">
      <span style="width:28px;height:28px;border-radius:50%;background:var(--bg-secondary);display:flex;align-items:center;justify-content:center;font-size:14px;">ğŸ‘¤</span>
      ${escHtml(name)}
    </div>`;
  }).join('') || `<div style="color:var(--text-muted);font-size:12px;padding:4px 0;">HenÃ¼z izleyici yok.</div>`;
}
...
function openViewerList() {
  ...
  panel.innerHTML = `
    <div style="font-weight:600;font-size:13px;margin-bottom:10px;">ğŸ‘ï¸ Ä°zleyiciler</div>
...
// Ã‡oklu stream grid
const _remoteScreenStreams = new Map(); // peerId â†’ { stream, el }
...
async function changeScreenShareQuality(quality) {
  if (!window.rtc?.screenSharing) return toast('Ekran paylaÅŸÄ±mÄ± aktif deÄŸil', 'error');
  try {
    await window.rtc.stopScreenShare?.();
    await new Promise(r => setTimeout(r, 300));
    const ok = await window.rtc.startScreenShare(quality, true);
    if (ok) {
      const label = document.getElementById('ss-quality-label');
      if (label) label.textContent = _qualityLabel(quality);
      toast(`Kalite deÄŸiÅŸtirildi: ${_qualityLabel(quality)} âœ…`, 'success');
      document.getElementById('ss-quality-change-panel')?.remove();
    }
  } catch { toast('Kalite deÄŸiÅŸtirilemedi', 'error'); }
}
...
function openQualityChangePanel() {
  ...
  panel.innerHTML = `
    <div style="font-weight:600;font-size:13px;margin-bottom:10px;text-align:center;">ğŸ–¥ï¸ Kalite DeÄŸiÅŸtir</div>
...
// â”€â”€ Go Live HTML enjeksiyonu â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function _injectGoLiveEnhancements() {
  ...
  viewerBtn.setAttribute('data-tip', 'Ä°zleyiciler');
  ...
  qualityBtn.setAttribute('data-tip', 'Kalite DeÄŸiÅŸtir');
  qualityBtn.onclick = openQualityChangePanel;
  qualityBtn.textContent = 'âš™ï¸ Kalite';
...
// Expose globals
window.addRemoteScreenStream      = addRemoteScreenStream;
window.removeRemoteScreenStream   = removeRemoteScreenStream;
window.openQualityChangePanel     = openQualityChangePanel;
window.changeScreenShareQuality   = changeScreenShareQuality;
window._onScreenShareStarted      = _onScreenShareStarted;
window._onScreenShareStopped      = _onScreenShareStopped;
