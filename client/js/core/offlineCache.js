...
(function initBridgeOfflineCache() {
  ...
  // ── v68: Offline Outbox — mesaj gönderme ───────────────────
  // Bağlantı yoksa SW outbox'a yazar, Background Sync ile gönderir.
  // Bağlantı varsa doğrudan gönderir.
  async function sendMessageWithOutbox(channelId, content, token, apiBase) {
    ...
    if (navigator.onLine) {
      try {
        ...
        // Network hatası — outbox'a ekle
      }
    }

    // Offline veya hata — SW outbox'a gönder
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.addEventListener('message', event => {
      if (event.data?.type === 'OUTBOX_FLUSHED') {
        ...
        if (remaining === 0) {
          window.bridgeApp?.toast('📤 Bekleyen mesajlar gönderildi', 'success');
        }
      }
    });
  }
...
// ─────────────────────────────────────────────────────────────
export const getBridgeOfflineCache = () => window.bridgeOfflineCache;
