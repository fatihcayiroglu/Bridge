// client/js/core/voice-recorder.js — Bridge Voice Recording
    ...
    try {
      ...
      if (this.options.noiseSuppression && window.BridgeNoiseSuppression) {
        try {
          ...
          console.warn('[VoiceRec] Noise suppression yüklenemedi, ham stream kullanılıyor:', nsErr);
          stream = rawStream;
        }
      }
...
class VoiceMessagePlayer {
   ...
   */
  constructor(voiceMsg, container) {
    this.vm = voiceMsg;
...
class VoiceRecorderUI {
  ...
  async _startRecording() {
    this.recorder = new BridgeVoiceRecorder({
      ...
      onStop: ({ duration }) => {
        console.log(`[VoiceUI] Kayıt durduruldu: ${duration}s`);
      },
      onUpload: ({ ok, data }) => {
...
// ─────────────────────────────────────────────────────────────
export const BridgeVoiceRecorder = window.BridgeVoiceRecorder;
export const VoiceMessagePlayer  = window.VoiceMessagePlayer;
export const VoiceRecorderUI     = window.VoiceRecorderUI;
export const getVoiceRecorder = () => window.BridgeVoiceRecorder;
