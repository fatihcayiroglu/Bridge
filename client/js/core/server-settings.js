...
// ─────────────────────────────────────────────────────────────
// Sunucu ayarları UI: emoji yönetimi, sunucu ayarları modalı,
// banner/ikon yükleme, webhook yönetimi, audit log export,
// SSO ayarları, plugin yönetimi.
//
// Önceki adı: core/api.js (yanlış isimlendirilmişti)
// apiFetch ve API sabiti: core/auth.js ve core/globals.js içinde
// ─────────────────────────────────────────────────────────────
...
async function openEmojiManager() {
  ...
  modal.innerHTML = `
    <div class="modal-card" style="max-width:520px;width:95%;max-height:80vh;display:flex;flex-direction:column;">
      <h2 style="margin-bottom:4px;">😀 Server Emojileri</h2>
      <p style="color:var(--text-muted);font-size:13px;margin-bottom:16px;">Nitro gerektirmez • Sunucuya özel • <strong style="color:var(--brand)">Sınırsız emoji</strong> • Cross-server kullanım</p>
      <div style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap;">
        <input type="text" id="new-emoji-name" class="input-field" placeholder="emoji_adı (a-z, 0-9, _)" style="flex:1;min-width:140px;" maxlength="32">
      ...
      <p style="font-size:11px;color:var(--text-muted);margin:-8px 0 12px;">PNG, GIF (animasyonlu!), WebP, JPEG • Max 256KB</p>
...
async function refreshEmojiGrid() {
  ...
  if (!grid) return;
  grid.innerHTML = '<div style="color:var(--text-muted);font-size:13px;padding:8px;">Yükleniyor...</div>';
  ...
  if (!emojis.length) {
    grid.innerHTML = '<div style="color:var(--text-muted);font-size:13px;padding:8px;grid-column:1/-1;">Henüz emoji yok. Yükle!</div>';
    return;
  }
  grid.innerHTML = '';
...
async function uploadServerEmoji(input) {
  ...
  const name = document.getElementById('new-emoji-name').value.trim().toLowerCase().replace(/[^a-z0-9_]/g,'_');
  if (!name) { toast('Emoji adı gir!', 'error'); input.value = ''; return; }
...
async function openServerSettings() {
  ...
  modal.innerHTML = `
    <div class="modal-card" style="max-width:480px;width:95%;max-height:85vh;overflow-y:auto;">
      <h2 style="margin-bottom:16px;">⚙️ Sunucu Ayarları</h2>
      ...
      <div style="background:var(--bg-primary);border-radius:10px;overflow:hidden;margin-bottom:20px;">
        <div id="server-banner-preview" style="height:100px;background:${server.bannerUrl ? 'url(' + API + encodeURI(server.bannerUrl) + ') center/cover' : 'linear-gradient(135deg,#5865f2,#3ba55c)'};position:relative;">
          ...
          ${server.bannerUrl ? `<button onclick="removeServerBanner()" style="position:absolute;bottom:6px;left:6px;background:rgba(0,0,0,.6);color:#fff;font-size:11px;padding:4px 8px;border-radius:6px;border:none;cursor:pointer;">🗑️ Kaldır</button>` : ''}
        </div>
        <div style="padding:12px;display:flex;align-items:center;gap:12px;">
          ...
          <div>
            ...
            <div style="font-size:11px;color:var(--text-muted);">İkon için tıkla</div>
          </div>
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <input type="text" id="srv-icon-input" class="input-field" value="${escHtml(server.icon || '🌐')}" maxlength="8">
      </div>

      <div class="form-group">
        <label>🌐 Herkese Açık Profil URL'si <span style="font-size:11px;color:var(--text-muted);font-weight:400">(opsiyonel)</span></label>
      ...
      <div style="border-top:1px solid var(--border);padding-top:16px;margin-top:8px;">
        ...
        <button class="btn" style="width:100%;margin-bottom:8px;justify-content:center;background:linear-gradient(135deg,#5865F2,#4752C4);color:#fff;" onclick="if(window.openDiscordImport){document.getElementById('server-settings-modal').remove();window.openDiscordImport();}else{alert('Discord Import modülü yüklenmedi');}">
          <span style="margin-right:6px">📥</span> Discord'dan İçe Aktar
        </button>
      </div>

      <div class="modal-footer" style="margin-top:16px;">
        <button class="btn btn-primary" onclick="saveServerSettings()">Kaydet</button>
        <button class="btn" onclick="document.getElementById('server-settings-modal').remove()">İptal</button>
      </div>
    </div>`;
  document.body.appendChild(modal);
  modal.addEventListener('click', e => { if (e.target === modal) modal.remove(); });
...
async function saveServerSettings() {
  ...
  const icon = document.getElementById('srv-icon-input').value.trim();
  if (!name) { toast('Sunucu adı boş olamaz', 'error'); return; }
...
async function openWebhookManager() {
  ...
  modal.innerHTML = `
    <div class="modal-card" style="max-width:520px;width:95%;max-height:85vh;overflow-y:auto;">
      <h2 style="margin-bottom:4px;">🔗 Webhook Yönetimi</h2>
      ...
      <div class="form-group">
        ...
        <div style="display:flex;gap:8px;margin-bottom:8px;">
          ...
          <input type="text" id="wh-name-input" class="input-field" placeholder="Webhook adı" maxlength="80" style="flex:1.5;">
          <button class="btn btn-primary" onclick="createWebhook()" style="white-space:nowrap;">+ Oluştur</button>
        </div>
      </div>
...
async function loadWebhookList() {
  ...
  if (!allWebhooks.length) {
    listEl.innerHTML = `<div style="color:var(--text-muted);font-size:13px;text-align:center;padding:20px;">Henüz webhook yok.</div>`;
    return;
  }
    ...
    <div class="webhook-item" style="background:var(--bg-primary);border-radius:8px;padding:12px;margin-bottom:8px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">
        ...
        <button class="btn" style="font-size:11px;padding:4px 10px;flex-shrink:0;color:var(--danger);"
          onclick="deleteWebhook('${escHtml(w.channelId)}','${escHtml(w._id)}')">🗑️ Sil</button>
      </div>
      <div style="margin-top:8px;display:flex;gap:6px;align-items:center;">
        ...
        <button class="btn" style="font-size:11px;padding:4px 10px;flex-shrink:0;"
          onclick="navigator.clipboard.writeText('${location.origin}${escHtml(w.url)}').then(()=>toast('Kopyalandı!','success'))">
          📋 Kopyala
        </button>
      </div>
    </div>`).join('');
...
async function openAuditLogExport() {
  ...
  modal.innerHTML = `
    <div class="modal-card" style="max-width:500px;width:95%;max-height:85vh;overflow-y:auto;">
      <h2 style="margin-bottom:16px;">📋 Audit Log Export</h2>
      ...
      <div class="form-group" style="margin-bottom:16px;">
        <label>Aksiyon Filtresi <span style="font-size:11px;color:var(--text-muted)">(boş = tümü)</span></label>
      ...
      <div style="margin-bottom:16px;">
        <label style="font-size:13px;font-weight:600;display:block;margin-bottom:6px;">Önizleme (son 10)</label>
        <div id="al-preview" style="background:var(--bg-1);border-radius:8px;padding:10px;max-height:200px;overflow-y:auto;font-size:12px;font-family:monospace;color:var(--text-2);">
          <div style="color:var(--text-muted)">Yüklemek için "Önizle" tıklayın</div>
        </div>
      </div>

      <div class="modal-footer" style="gap:8px;flex-wrap:wrap;">
        <button class="btn btn-primary" onclick="alPreview()">👁️ Önizle</button>
        <button class="btn btn-primary" onclick="alExport('csv')" style="background:var(--green)">⬇️ CSV İndir</button>
        <button class="btn btn-primary" onclick="alExport('json')" style="background:#f0a020">⬇️ JSON İndir</button>
...
async function alPreview() {
  ...
  if (!el || !srv) return;
  el.innerHTML = '<div style="color:var(--text-muted)">Yükleniyor…</div>';
  try {
    const qs = _alBuildParams({ limit: 10 });
    const r  = await apiFetch(`${API}/api/servers/${srv._id}/audit-log?${qs}`);
    const d  = await r.json();
    const logs = d.logs || d;
    if (!logs.length) { el.innerHTML = '<div style="color:var(--text-muted)">Kayıt bulunamadı</div>'; return; }
    el.innerHTML = logs.map(l => {
      const dt = new Date(l.createdAt).toLocaleString('tr-TR');
      return `<div style="border-bottom:1px solid var(--bg-5);padding:4px 0;"><span style="color:var(--text-3)">${escHtml(dt)}</span> <b>${escHtml(l.action || '')}</b> ${escHtml(l.actorName || '')} → ${escHtml(l.targetName || '')}</div>`;
    }).join('');
  } catch { el.innerHTML = '<div style="color:var(--red)">Yükleme başarısız</div>'; }
}
...
async function openSSOSettings() {
  ...
  const oidcEnabled = cfg.oidc?.enabled ? '✅ Aktif' : '⛔ Pasif';
  const samlEnabled = cfg.saml?.enabled ? '✅ Aktif' : '⛔ Pasif';

  modal.innerHTML = `
    <div class="modal-card" style="max-width:500px;width:95%;max-height:85vh;overflow-y:auto;">
      <h2 style="margin-bottom:16px;">🔐 SSO / Kurumsal Giriş</h2>
...
OIDC_SCOPES=openid email profile</pre>
        <div style="margin-top:8px;font-size:12px;color:var(--text-muted)">
          Giriş URL: <code style="background:var(--bg-3);padding:2px 6px;border-radius:4px;">/api/sso/oidc/start</code>
        </div>
      </div>
...
SAML_ISSUER=https://bridge.example.com</pre>
      ...
      <div style="background:var(--bg-3);border-radius:8px;padding:10px;margin-bottom:16px;font-size:12px;color:var(--text-muted);">
        💡 SSO ile giriş yapan kullanıcılar şifre olmadan hesap oluşturur. E-posta eşleşmesi ile mevcut hesaplara SSO bağlanır.
      </div>
...
async function openPluginManager() {
  ...
  modal.innerHTML = `
    <div class="modal-card" style="max-width:520px;width:95%;max-height:85vh;overflow-y:auto;">
      <h2 style="margin-bottom:4px;">🧩 Plugin Yönetimi</h2>
      <p style="color:var(--text-muted);font-size:13px;margin-bottom:16px;">Sunucuda yüklü aktif plugin'ler</p>
      <div id="plugin-list">
      ...
      <div style="background:var(--bg-1);border-radius:8px;padding:12px;margin-top:16px;font-size:12px;color:var(--text-muted);">
        💡 Plugin eklemek için <code style="background:var(--bg-3);padding:1px 5px;border-radius:4px;">plugins/</code> klasörüne yeni bir dizin ekleyin ve sunucuyu yeniden başlatın.
        <a href="https://github.com/bridge/bridge/blob/main/plugins/README.md" target="_blank" style="color:var(--brand);display:block;margin-top:4px;">📖 Plugin Geliştirme Kılavuzu →</a>
      </div>
      <div class="modal-footer" style="margin-top:16px;">
  ...
  try {
    ...
    if (!plugins.length) {
      el.innerHTML = '<div style="text-align:center;padding:24px;color:var(--text-muted);">Yüklü plugin bulunamadı</div>';
      return;
    }
    el.innerHTML = plugins.map(p => `
      <div style="background:var(--bg-1);border-radius:10px;padding:14px;margin-bottom:10px;display:flex;gap:12px;align-items:flex-start;">
        ...
        <div style="flex:1;min-width:0;">
          ...
          <div style="font-size:11px;color:var(--brand);margin-bottom:4px;">v${escHtml(p.version || '?')} · ${escHtml(p.author || 'Bilinmeyen')}</div>
          ${p.description ? `<div style="font-size:12px;color:var(--text-muted);">${escHtml(p.description)}</div>` : ''}
        </div>
        <span style="background:var(--green);color:#fff;font-size:10px;font-weight:700;padding:2px 8px;border-radius:10px;flex-shrink:0;">AKTİF</span>
      </div>`).join('');
  } catch {
    const el = document.getElementById('plugin-list');
    if (el) el.innerHTML = '<div style="color:var(--red);padding:16px">Plugin listesi alınamadı</div>';
  }
}
