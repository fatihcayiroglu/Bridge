...
// ModÃ¼l: Mesaj Zamanlama gÃ¶rsel takvim picker
'use strict';
  ...
  function render() {
    ...
    picker.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
        <button class="cal-nav-btn" onclick="calPickNav(-1)">â€¹</button>
        <strong>${months[viewMonth]} ${viewYear}</strong>
        <button class="cal-nav-btn" onclick="calPickNav(1)">â€º</button>
      </div>
      ...
      <div style="margin-top:14px;display:flex;align-items:center;gap:10px;justify-content:center;">
        <label style="font-size:13px;">ğŸ• Saat:</label>
      ...
      <div style="display:flex;gap:8px;margin-top:14px;justify-content:center;">
        <button class="btn btn-primary" onclick="calPickConfirm('${targetInputId}')">âœ“ SeÃ§</button>
        <button class="btn" onclick="document.getElementById('calendar-picker-popup').remove()">Ä°ptal</button>
      </div>`;
    updateCalPreview();
  }
