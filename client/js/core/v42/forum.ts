...
// ModÃ¼l: Forum tag filtre + sÄ±ralama + pin/kilit kartlarÄ±
'use strict';
export {};
...
window.loadForumChannel = async function(channelId) {
  ...
  if (!area) return;
  area.innerHTML = '<div style="padding:40px;text-align:center;color:var(--text-3)">YÃ¼kleniyor...</div>';
  ...
  area.innerHTML = `
    <div class="forum-header" style="border-bottom:1px solid var(--border);padding-bottom:16px;margin-bottom:0;">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
        <div>
          <h2 style="font-size:20px;font-weight:700;color:var(--text-1);margin:0">ğŸ“‹ ${escHtml(window.currentChannel?.name || '')}</h2>
          <p style="color:var(--text-3);font-size:13px;margin:4px 0 0">${escHtml(window.currentChannel?.topic || 'Forum kanalÄ±')}</p>
        </div>
        <button class="btn btn-primary" onclick="openNewForumThread()" style="gap:6px">âœï¸ Yeni Ä°leti AÃ§</button>
      </div>
      <div style="display:flex;align-items:center;gap:8px;margin-top:14px;flex-wrap:wrap;">
        <div style="display:flex;gap:4px;">
          <button class="forum-sort-btn ${_forumSort === 'activity' ? 'active' : ''}" onclick="setForumSort('activity')">ğŸ• Son Aktivite</button>
          <button class="forum-sort-btn ${_forumSort === 'new' ? 'active' : ''}" onclick="setForumSort('new')">âœ¨ En Yeni</button>
          <button class="forum-sort-btn ${_forumSort === 'top' ? 'active' : ''}" onclick="setForumSort('top')">ğŸ”¥ En Aktif</button>
        </div>
        <div style="display:flex;gap:4px;flex-wrap:wrap;" id="forum-tag-filters">
          ...
          `).join('')}
          ${_forumAvailTags.length ? `<button class="forum-tag-btn clear-tag" onclick="setForumTagFilter(null)" style="display:none">âœ• Temizle</button>` : ''}
        </div>
      </div>
    </div>
...
function renderForumGridV42() {
  ...
  if (!threads.length) {
    grid.innerHTML = `<div style="text-align:center;padding:60px 20px;color:var(--text-3)">
      ...
      <div style="font-size:16px;font-weight:600">${_forumActiveTag ? `"${_forumActiveTag}" etiketinde ileti yok` : 'HenÃ¼z ileti yok'}</div>
    </div>`;
    return;
  }

  grid.innerHTML = '';
  const isMod = window.currentServer?.ownerId === window.me?.id;
      ...
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;">
        <div style="min-width:0;flex:1;">
          <div class="forum-card-title">
            ${t.pinned ? '<span title="SabitlenmiÅŸ" style="font-size:12px;margin-right:4px;">ğŸ“Œ</span>' : ''}
            ${t.locked ? '<span title="Kilitli" style="font-size:12px;margin-right:4px;">ğŸ”’</span>' : ''}
            ${escHtml(t.name || 'Ä°simsiz ileti')}
          </div>
          <div class="forum-card-preview">${escHtml((t.firstMessage || '').slice(0, 120))}${(t.firstMessage||'').length > 120 ? 'â€¦' : ''}</div>
        ...
        ${isMod ? `<div class="forum-mod-actions" onclick="event.stopPropagation()">
          <button class="forum-mod-btn" title="${t.pinned ? 'Sabitlemeyi kaldÄ±r' : 'Sabitle'}" onclick="forumTogglePin('${escHtml(t._id)}', ${!t.pinned})">${t.pinned ? 'ğŸ“Œ' : 'ğŸ“'}</button>
          <button class="forum-mod-btn" title="${t.locked ? 'Kilidi aÃ§' : 'Kilitle'}" onclick="forumToggleLock('${escHtml(t._id)}', ${!t.locked})">${t.locked ? 'ğŸ”“' : 'ğŸ”’'}</button>
          <button class="forum-mod-btn" title="Etiket ekle" onclick="forumEditTags('${escHtml(t._id)}')">ğŸ·ï¸</button>
        </div>` : ''}
      </div>
...
async function forumTogglePin(threadId, pinned) {
  ...
  if (!r.ok) return toast('Ä°ÅŸlem baÅŸarÄ±sÄ±z', 'error');
  const t = _forumAllThreads.find(x => x._id === threadId);
  if (t) t.pinned = pinned;
  toast(pinned ? 'Ä°leti sabitlendi ğŸ“Œ' : 'Sabitleme kaldÄ±rÄ±ldÄ±', 'success');
  renderForumGridV42();
}
...
async function forumToggleLock(threadId, locked) {
  ...
  toast(locked ? 'Ä°leti kilitlendi ğŸ”’' : 'Kilit aÃ§Ä±ldÄ± ğŸ”“', 'success');
  renderForumGridV42();
}
...
function forumEditTags(threadId) {
  ...
  const input = prompt('Etiketleri dÃ¼zenle (virgÃ¼lle ayÄ±r, max 5):', current);
  ...
  apiFetch(`${API}/api/threads/${threadId}`, {
    ...
    _forumAvailTags = [...tagSet];
    toast('Etiketler gÃ¼ncellendi ğŸ·ï¸', 'success');
    renderForumGridV42();
  });
}
...
window.openNewForumThread = function() {
  ...
  modal.innerHTML = `
    <div class="modal-card" style="max-width:500px;width:95%">
      <h2>âœï¸ Yeni Ä°leti AÃ§</h2>
      <div class="form-group">
        <label>BaÅŸlÄ±k</label>
        <input type="text" id="forum-title-input" class="input-field" placeholder="Ä°leti baÅŸlÄ±ÄŸÄ±..." maxlength="100">
      </div>
      <div class="form-group">
        <label>Ä°Ã§erik</label>
        <textarea id="forum-body-input" class="input-field" rows="4" placeholder="Ne hakkÄ±nda konuÅŸmak istiyorsun?" style="resize:vertical"></textarea>
      </div>
      <div class="form-group">
        <label>Etiketler <span style="font-weight:400;color:var(--text-muted);font-size:11px">(virgÃ¼lle ayÄ±r, max 5 â€” Ã¶r: Ã¶neri, hata, duyuru)</span></label>
        <input type="text" id="forum-tags-input" class="input-field" placeholder="Ã¶neri, hata, soru">
      </div>
  ...
  document.getElementById('forum-submit-btn').onclick = async () => {
    const name  = (document.getElementById('forum-title-input') as HTMLInputElement | null)?.value ?? ''.trim();
    const body  = (document.getElementById('forum-body-input') as HTMLInputElement | null)?.value ?? ''.trim();
    const tagsRaw = (document.getElementById('forum-tags-input') as HTMLInputElement | null)?.value ?? '';
    ...
    const r = await apiFetch(`${API}/api/threads`, {
      ...
      body: JSON.stringify({ channelId: window.currentChannel?._id, name, firstMessage: body, tags }),
    });
    const data = await r.json();
    if (!r.ok) return toast(data.error || 'OluÅŸturulamadÄ±', 'error');
    _destroyTempModal();
    toast('Ä°leti aÃ§Ä±ldÄ±!', 'success');
    await window.loadForumChannel(window.currentChannel._id);
...
window.setForumTagFilter  = setForumTagFilter;
window.renderForumGridV42 = renderForumGridV42;
window.forumTogglePin     = forumTogglePin;
window.forumToggleLock    = forumToggleLock;
window.forumEditTags      = forumEditTags;
