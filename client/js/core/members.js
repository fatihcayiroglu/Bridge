...
async function loadMembers(serverId) {
  ...
  () = members; // cache for mention autocomplete
...
function _memberContextMenu(e, uid, name, color, isOnline) {
  ...
  const items = [];
  if (!isMe) {
    items.push({ icon:'💬', label:'Mesaj Gönder',          action:`openDmWithUser('${uid}','${name}','${color}')` });
    if (isOnline) {
      items.push({ icon:'📞', label:'Ses Araması Yap',      action:`_memberQuickCall('${uid}','${name}','${color}')` });
      items.push({ icon:'📹', label:'Görüntülü Ara',        action:`openDmWithUser('${uid}','${name}','${color}');setTimeout(()=>DmCall?.startCall('${uid}','${name}','${color}','video'),300)` });
    }
    items.push({ icon:'👤', label:'Profili Gör',            action:`openProfileModal && openProfileModal('${uid}')` });
    items.push({ icon:'✏️',  label:'Takma Ad Ayarla',       action:`_setNicknamePrompt('${uid}','${name}')` });
    items.push({ icon:'🚫', label:'Engelle',                action:`_blockUser('${uid}','${name}')`, danger: true });
    items.push({ icon:'📋', label:'Kullanıcı Adı Kopyala', action:`navigator.clipboard.writeText('${name}').then(()=>toast('Kopyalandı','success'))` });
    ...
    items.push({ icon:'✏️',  label:'Takma Adımı Sıfırla', action:`_setNicknamePrompt('${uid}','${name}')` });
  }
