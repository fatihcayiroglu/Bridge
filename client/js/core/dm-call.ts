// core/dm-call.js
// DM Ses & Video AramasÄ± (1-1 WebRTC over Socket.IO)
...
const DmCall = (() => {
  ...
  // â”€â”€ Overlay HTML â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    el.innerHTML = `
      <div id="dm-call-box">
        ...
        <div id="dm-call-actions">
          <button class="dm-call-btn dm-call-btn-red"   id="dm-call-hangup"  onclick="DmCall.hangUp()"      title="Kapat">ğŸ“µ</button>
          <button class="dm-call-btn dm-call-btn-gray"  id="dm-call-mute"    onclick="DmCall.toggleMic()"   title="Mikrofon">ğŸ¤</button>
          <button class="dm-call-btn dm-call-btn-gray"  id="dm-call-cam"     onclick="DmCall.toggleCam()"   title="Kamera" style="display:none">ğŸ“·</button>
          <button class="dm-call-btn dm-call-btn-gray"  id="dm-call-screen"  onclick="DmCall.toggleScreen()" title="Ekran PaylaÅŸ" style="display:none">ğŸ–¥ï¸</button>
        </div>
        <div id="dm-call-incoming-actions" style="display:none">
          <button class="dm-call-btn dm-call-btn-green" id="dm-call-accept"  onclick="DmCall.accept()"   title="Kabul">ğŸ“</button>
          <button class="dm-call-btn dm-call-btn-red"   id="dm-call-reject"  onclick="DmCall.decline()"  title="Reddet">ğŸ“µ</button>
        </div>
      </div>
    `;
    // Styles
  ...
  async function _initPC() {
    ...
    _pc.onconnectionstatechange = () => {
      if (_pc.connectionState === 'connected') _setStatus(_currentType === 'video' ? 'ğŸ“¹ GÃ¶rÃ¼ntÃ¼lÃ¼ arama aktif' : 'ğŸ¤ Ses aramasÄ± aktif');
  ...
  async function _getMedia(withVideo) {
    ...
    if (!_localStream) { toast('Mikrofona eriÅŸilemiyor', 'error'); return false; }
  ...
  // â”€â”€ Screen Share inside DM call â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  async function toggleScreen() {
    ...
    try {
      ...
      const btn = document.getElementById('dm-call-screen');
      if (btn) { btn.textContent = 'â¹ï¸'; btn.classList.add('active'); }
    } catch (e) {
      if (e.name !== 'NotAllowedError') console.warn('Screen share error:', e);
    }
  }

  // â”€â”€ Start call (caller side) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  async function startCall(toUserId, displayName, avatarColor, type = 'voice') {
    ...
    _setStatus(type === 'video' ? 'ğŸ“¹ GÃ¶rÃ¼ntÃ¼lÃ¼ arama kuruluyorâ€¦' : 'ğŸ“ AranÄ±yorâ€¦');
  ...
  // â”€â”€ Incoming call (callee side) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  function _handleIncoming({ callId, type, callerId, callerDisplayName, callerAvatarColor }) {
    ...
    _setStatus(type === 'video' ? 'ğŸ“¹ GÃ¶rÃ¼ntÃ¼lÃ¼ arama geliyorâ€¦' : 'ğŸ“ Ses aramasÄ± geliyorâ€¦');
  ...
  function toggleMic() {
    ...
    if (btn) { btn.textContent = track.enabled ? 'ğŸ¤' : 'ğŸ”‡'; btn.classList.toggle('active', !track.enabled); }
  }
  ...
  function toggleCam() {
    ...
    if (btn) { btn.textContent = track.enabled ? 'ğŸ“·' : 'ğŸš«'; btn.classList.toggle('active', !track.enabled); }
  }
  ...
  // â”€â”€ Socket event bindings (called after socket ready) â”€â”€â”€â”€â”€
  function bindSocketEvents(sock) {
    ...
    sock.on('dm:call:accepted', async ({ callId, type, calleeDisplayName }) => {
      _setStatus(`${calleeDisplayName} kabul etti â€” baÄŸlanÄ±yorâ€¦`);
    });
    ...
    sock.on('dm:call:ended', ({ callId }) => {
      _setStatus('Arama sonlandÄ±');
      setTimeout(_cleanup, 1200);
    });

    sock.on('dm:call:missed', ({ callId }) => {
      if (_role === 'caller') { _setStatus('Cevap alÄ±namadÄ±'); setTimeout(_cleanup, 2000); }
      else _cleanup();
    });
  }
