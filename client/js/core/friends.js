...
// misc.js'den çıkarıldı: Arkadaş sistemi, voice activity, status
...
function renderFriendsTab(tab) {
  ...
  if (tab === 'pending') {
    ...
    list.innerHTML = displayed.map(f => {
      ...
      const statusLabels = { online: 'Çevrimiçi', idle: 'Uzakta', dnd: 'Rahatsız Etme', offline: 'Çevrimdışı' };
        ...
        <div class="friend-item">
          ...
          <div class="friend-info">
            ...
            <div class="friend-status" style="color:${statusColors[f.status] || 'var(--text-muted)'}">
              ${statusLabels[f.status] || 'Çevrimdışı'}${escHtml(activity)}
            </div>
          </div>
          <div style="display:flex;gap:6px">
