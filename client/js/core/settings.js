...
async function uploadServerGif() {
  ...
  document.getElementById('server-gif-name').value = '';
  document.getElementById('server-gif-tags').value = '';
...
const SC_STATUS_COLORS = { stopped:'#747f8d', starting:'#faa61a', running:'#23a55a', error:'#ed4245' };
const SC_STATUS_LABELS = { stopped:'Durduruldu', starting:'Başlatılıyor…', running:'Çalışıyor', error:'Hata' };

function scUpdateUI(status, pid) {
  ...
  const btnRestart = document.getElementById('sc-btn-restart');
...
let _scStatusCb = null;
let _scLogCb    = null;

async function openServerControlPanel() {
  const sc = window.serverControl;
  if (!sc) { toast('Sunucu kontrolü sadece masaüstü uygulamasında kullanılabilir.', 'error'); return; }
  ...
  const area = document.getElementById('sc-log-area');
  if (area && logs?.length) { area.innerHTML = ''; logs.forEach(scAppendLog); }

  if (_scStatusCb) sc.offStatus(_scStatusCb);
  if (_scLogCb)    sc.offLog(_scLogCb);

  _scStatusCb = (d) => scUpdateUI(d.status, d.pid);
  _scLogCb    = (d) => scAppendLog(d);

  sc.onStatus(_scStatusCb);
  sc.onLog(_scLogCb);
}

function scStart()   { window.serverControl?.start(); }
function scStop()    { window.serverControl?.stop(); }
function scRestart() { window.serverControl?.restart(); }
...
document.addEventListener('DOMContentLoaded', () => {
  if (!window.serverControl) return;
