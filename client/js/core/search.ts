...
// GeliÅŸmiÅŸ arama â€” filtre chip'leri, tarih aralÄ±ÄŸÄ±,
//      from:kullanÄ±cÄ±, has:file/image/link, sayfalama

'use strict';
export {};

// â”€â”€ ARAMA DURUMU â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
interface GsFilters {
  has?:    string;
  from?:   string;
  after?:  string;
...
let gsTab:     string   = 'all';
let gsTimer:   ReturnType<typeof setTimeout> | null = null;
let gsFilters: GsFilters = {};  // { has: 'file', from: 'aliid', after: '...', before: '...' }
let gsFromMap: Record<string, string> = {};          // userId â†’ displayName (chip label iÃ§in)
let gsPage    = 0;
const GS_PAGE_SIZE = 25;

// â”€â”€ CTRL+K KLAVYE KISAYOLU â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function switchGsTab(tab, btn) {
  ...
  // HÄ±zlÄ± filtreler sadece mesaj aramada anlamlÄ±
...
function onGlobalSearch(val) {
  ...
  if (!val.trim()) {
    ...
    if (res) res.innerHTML = '<div class="gs-empty">Aramak iÃ§in yazmaya baÅŸlayÄ±nâ€¦<br><span class="gs-hint">Ä°pucu: <code>from:ali</code> Â· <code>has:file</code> Â· <code>before:2025-06-01</code></span></div>';
...
async function doGlobalSearch(q) {
  ...
  const res = document.getElementById('gs-results');
  if (!res) return;
  res.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-muted)">AranÄ±yorâ€¦</div>';

  try {
    // Filtre state'ini URL param'larÄ±na dÃ¶nÃ¼ÅŸtÃ¼r
    ...
    if (!r.ok) {
      res.innerHTML = `<div style="color:var(--red);padding:16px">${escHtml(data.error || 'Arama baÅŸarÄ±sÄ±z')}</div>`;
      return;
    }
    ...
    res.innerHTML = `<div style="color:var(--red);padding:16px">BaÄŸlantÄ± hatasÄ±</div>`;
  }
}

// â”€â”€ v69: Sayfalama â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ v69: Filtreler â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function gsClearFilters() {
  ...
  gsFromMap  = {};
  gsPage     = 0;
  { const _el = document.getElementById('gs-from-input') as HTMLInputElement | null; if (_el) _el.value = ''; }
  { const _el = document.getElementById('gs-date-after') as HTMLInputElement | null; if (_el) _el.value = ''; }
  { const _el = document.getElementById('gs-date-before') as HTMLInputElement | null; if (_el) _el.value = ''; }
  gsClosePickers();
  gsRenderChips();
  gsUpdateQuickFilterButtons();
  const q = document.getElementById('gs-input')?.value?.trim();
...
function gsRenderChips() {
  ...
  const chips = [];
  if (gsFilters.has)   chips.push({ label: `ğŸ“ ${gsFilters.has}`,                    key: 'has'   });
  if (gsFilters.from)  chips.push({ label: `ğŸ‘¤ ${gsFromMap[gsFilters.from] || gsFilters.from}`, key: 'from'  });
  if (gsFilters.after) chips.push({ label: `ğŸ“… ${gsFilters.after} sonrasÄ±`,           key: 'after' });
  if (gsFilters.before)chips.push({ label: `ğŸ“… ${gsFilters.before} Ã¶ncesi`,           key: 'before'});

  container.style.display = chips.length ? 'flex' : 'none';
  if (clearBtn) clearBtn.style.display = chips.length ? '' : 'none';
  container.innerHTML = chips.map(c =>
    `<span class="gs-chip">${escHtml(c.label)}<button onclick="gsRemoveFilter('${c.key}')">Ã—</button></span>`
  ).join('');
}
...
// â”€â”€ v69: KiÅŸi seÃ§ici â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ v69: Tarih seÃ§ici â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function renderGsResults(data, q) {
  ...
  // Filtreleri sorgu metninden Ã§Ä±kar (pure arama kelimesi iÃ§in highlight)
  ...
  if (!msgs.length && !members.length && !chans.length) {
    ...
    res.innerHTML = `<div style="text-align:center;padding:32px;color:var(--text-muted);">
      ...
      <p>"${escHtml(pureQ || q)}" iÃ§in sonuÃ§ bulunamadÄ±</p>
  ...
  // â”€â”€ Kanallar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  if (chans.length) {
    html += `<div class="gs-section-label">ğŸ“¢ Kanallar (${chans.length})</div>`;
    html += chans.map(c => `
  ...
  // â”€â”€ Ãœyeler â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  if (members.length) {
    html += `<div class="gs-section-label">ğŸ‘¥ Ãœyeler (${members.length})</div>`;
    html += members.map(m => `
      <div class="gs-item" onclick="openDmWithUser('${m._id}','${escHtml(m.displayName)}','${m.avatarColor || '#4f8ef7'}');closeGlobalSearch()">
        ...
        <button class="gs-action-btn" onclick="event.stopPropagation();gsFilterFrom('${m._id}','${escHtml(m.displayName)}')" title="Bu kiÅŸiden mesajlarÄ± ara">ğŸ”</button>
      </div>`).join('');
  }

  // â”€â”€ Mesajlar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  if (msgs.length) {
    html += `<div class="gs-section-label">ğŸ’¬ Mesajlar (${msgs.length}${data.hasMore ? '+' : ''})</div>`;
    html += msgs.map(m => {
      ...
      const badge   = hasImg ? '<span class="gs-att-badge">ğŸ–¼ï¸</span>' : hasFile ? '<span class="gs-att-badge">ğŸ“</span>' : '';
      ...
      return `
        <div class="gs-item gs-msg-item" onclick="jumpToMessage('${m._id}','${m.channelId}','${currentServer._id}');closeGlobalSearch()">
          ...
          <button class="gs-action-btn" onclick="event.stopPropagation();copyToClipboard('${escHtml(m.content?.slice(0,200)||'')}');" title="Ä°Ã§eriÄŸi kopyala">ğŸ“‹</button>
        </div>`;
    }).join('');
  }
...
// "Bu kiÅŸiden ara" kÄ±sayolu (Ã¼ye listesindeki buton)
...
// â”€â”€ PIN PANEL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function openPinsPanel() {
  ...
  panel.innerHTML = `
    <div class="pins-header">
      <span>ğŸ“Œ SabitlenmiÅŸ Mesajlar</span>
      <button class="icon-btn" onclick="closePinsPanel()">âœ•</button>
    </div>
    <div id="pins-list" class="pins-list">
      <div style="text-align:center;padding:20px;color:var(--text-muted)">YÃ¼kleniyor...</div>
    </div>`;
  ...
  try {
    ...
    if (!list) return;
    if (!pins.length) {
      list.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-muted)">SabitlenmiÅŸ mesaj yok</div>';
      return;
    }
    ...
    const list = document.getElementById('pins-list');
    if (list) list.innerHTML = '<div style="color:var(--red);padding:16px">YÃ¼klenemedi</div>';
  }
}
...
// â”€â”€ DRAFT SAVE/RESTORE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
