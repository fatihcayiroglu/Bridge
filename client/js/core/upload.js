...
// Sunucuyla senkron — server/routes/upload.js ALLOWED_TYPES ile eşleşmeli
...
async function openFilePicker() {
  ...
  inp.onchange = async (e) => {
    ...
    // ── Client-side MIME kontrolü — sunucuya göndermeden önce erken reddet ──
    if (file.type && !_ALLOWED_MIME_TYPES.has(file.type)) {
      return toast(`Bu dosya türü desteklenmiyor: ${file.type}`, 'error');
    }

    // ── Boyut kontrolü — paste yoluyla gelen büyük dosyalar dahil ──
    const maxBytes = (clientConfig.maxFileSizeMB || 2048) * 1024 * 1024;
    if (file.size > maxBytes) return toast(`Dosya çok büyük (max ${clientConfig.maxFileSizeMB}MB)`, 'error');
...
function showUploadProgress(fileName, pct) {
  ...
  el.innerHTML = `<div class="upload-progress-info"><span class="upload-filename">📤 ${escHtml(fileName.slice(0,40))}</span><span class="upload-pct">${pct}%</span></div><div class="upload-progress-track"><div class="upload-progress-fill" style="width:${pct}%"></div></div>`;
...
// ── PASTE dosya/görsel yükleme ────────────────────────────────
// Pano'dan yapıştırılan görsel veya dosyayı yakala — metin değil.
async function handleMsgPaste(event) {
  ...
  for (const item of items) {
    ...
    // MIME kontrolü
    if (file.type && !_ALLOWED_MIME_TYPES.has(file.type)) {
      toast(`Bu dosya türü desteklenmiyor: ${file.type}`, 'error');
    ...
    if (file.size > maxBytes) {
      toast(`Yapıştırılan dosya çok büyük (max ${clientConfig.maxFileSizeMB}MB)`, 'error');
      event.preventDefault();
      return;
    }

    if (!currentChannel || currentChannel.type !== 'text') return;
    event.preventDefault(); // textarea'ya ham yapıştırmayı engelle
    ...
    return; // ilk dosyayı işle, durakla
  }
}
