...
// Sesli mesaj kaydÄ± ve gÃ¶nderimi
...
async function startVoiceRecord() {
  ...
  try {
    ...
    toast('ğŸ¤ KayÄ±t baÅŸladÄ±... BÄ±rakÄ±nca gÃ¶nderilir', '');
  } catch {
    toast('Mikrofon eriÅŸimi reddedildi', 'error');
  }
}
...
async function sendVoiceMessage() {
  ...
  if (blob.size < 1000) return toast('KayÄ±t Ã§ok kÄ±sa', 'error');
  ...
  try {
    ...
    toast('ğŸ¤ Sesli mesaj gÃ¶nderildi', 'success');
  } catch (e) {
    toast('Sesli mesaj gÃ¶nderilemedi: ' + e.message, 'error');
  }
}
