// client/js/core/voice-recorder.js â€” Bridge Voice Recording
// MediaRecorder API ile ses kaydÄ±, server upload, dinleme UI
//
// Ã–zellikler:
//   - TarayÄ±cÄ± MediaRecorder API (WebM/Opus codec)
//   - GerÃ§ek zamanlÄ± ses seviyesi gÃ¶rselleÅŸtirme (waveform)
...
//   - Stage â†’ Podcast akÄ±ÅŸÄ± iÃ§in kayÄ±t desteÄŸi
...
class BridgeVoiceRecorder {
  ...
  constructor(options: any = {}) {
    this.options = {
      ...
      onError: options.onError || ((e) => console.error('[VoiceRec]', e)),
    };
  ...
  // â”€â”€ Codec SeÃ§imi â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  _bestMimeType() {
    const candidates = [
  ...
  // â”€â”€ KayÄ±t BaÅŸlat â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  async start() {
    ...
    try {
      ...
      if (this.options.noiseSuppression && window.BridgeNoiseSuppression) {
        try {
          this._noiseSuppressor = new window.BridgeNoiseSuppression();
          await this._noiseSuppressor.init();
          stream = await this._noiseSuppressor.process(rawStream);
        } catch (nsErr) {
          console.warn('[VoiceRec] Noise suppression yÃ¼klenemedi, ham stream kullanÄ±lÄ±yor:', nsErr);
          stream = rawStream;
        }
      }
      ...
      // Her 250ms'de chunk al (smooth upload iÃ§in)
      this._mediaRecorder.start(250);
      this.state = 'recording';

      // Maksimum sÃ¼re timer
  ...
  // â”€â”€ KayÄ±t Durdur â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  stop() {
  ...
  // â”€â”€ KayÄ±t Ä°ptal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  cancel() {
    if (this.state === 'idle') return;
    clearTimeout(this._maxTimer);
    this._stopAnalyser();
  ...
  // â”€â”€ KayÄ±t Bitti â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  async _upload(blob, duration) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Upload baÅŸarÄ±sÄ±z: ${res.status}`);
  ...
  // â”€â”€ Waveform Analyser â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  drawWaveform(canvas) {
    ...
    const draw = () => {
      ...
      data.forEach((value) => {
        ...
        const hue = 220 + (value / 255) * 60; // mavi â†’ mor
...
// VoiceMessage Player â€” Ses mesajÄ± dinleme UI bileÅŸeni
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class VoiceMessagePlayer {
   ...
   */
  constructor(voiceMsg: any, container: HTMLElement) {
    this.vm = voiceMsg;
    ...
    this.container.innerHTML = `
      ...
      ">
        <button class="vm-play-btn" style="
          ...
          background:var(--accent,#5865f2); border:none;
        ...
        <div style="flex:1; min-width:0;">
          <div class="vm-waveform-container" style="position:relative; height:32px; cursor:pointer;">
            ...
            <div class="vm-progress-overlay" style="
              ...
              background:rgba(88,101,242,0.3); width:0%;
...
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// VoiceRecorderUI â€” Sohbet Ã§ubuÄŸuna entegre UI
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class VoiceRecorderUI {
  ...
  _init() {
    // Mikrofon butonunu oluÅŸtur
    this._btn = document.createElement('button');
    this._btn.className = 'vm-record-btn';
    this._btn.title = 'Ses mesajÄ± gÃ¶nder';
    this._btn.innerHTML = `
  ...
  async _startRecording() {
    this.recorder = new BridgeVoiceRecorder({
      ...
      onStop: ({ duration }) => {
        console.log(`[VoiceUI] KayÄ±t durduruldu: ${duration}s`);
      },
      onUpload: ({ ok, data }) => {
        this._hideRecordingUI();
        if (!ok) this._showError('Upload baÅŸarÄ±sÄ±z');
      },
      onError: (err) => {
        this._hideRecordingUI();
        if (err.name === 'NotAllowedError') {
          this._showError('Mikrofon eriÅŸimi reddedildi');
        } else {
          this._showError('Ses kaydÄ± baÅŸlatÄ±lamadÄ±');
        }
      },
    });
  ...
  _showRecordingUI() {
    ...
    this._btn.title = 'GÃ¶nder (tekrar tÄ±kla)';
  ...
  /**
   * Kanal deÄŸiÅŸtir (sayfa navigasyonunda)
   */
...
// â”€â”€ CSS Animasyon â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
if (!document.getElementById('vm-styles')) {
  ...
  style.textContent = `
    ...
    .vm-play-btn:hover { background: var(--accent-hover, #4752c4) !important; }
    .vm-waveform-container:hover .vm-progress-overlay { background: rgba(88,101,242,0.45); }
  `;
  document.head.appendChild(style);
}

// â”€â”€ Global Export â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
window.BridgeVoiceRecorder = BridgeVoiceRecorder;
window.VoiceMessagePlayer = VoiceMessagePlayer;
window.VoiceRecorderUI = VoiceRecorderUI;

// â”€â”€ Otomatik init (eÄŸer chat input varsa) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
document.addEventListener('DOMContentLoaded', () => {
  // Mevcut mesaj giriÅŸ alanÄ±nÄ±n yanÄ±na mikrofon butonu ekle
  const init = () => {
    ...
    const channelId = window.currentChannelId || null;
    const serverId = window.currentServerId || null;

    window._voiceRecorderUI = new VoiceRecorderUI({ channelId, serverId });
    window._voiceRecorderUI.mount(inputArea);
  };

  // Socket event'leri dinle â€” kanal deÄŸiÅŸtiÄŸinde gÃ¼ncelle
  init();
  document.addEventListener('bridge:channel-changed', (e) => {
    const { channelId, serverId } = e.detail || {};
    if (window._voiceRecorderUI) {
      window._voiceRecorderUI.setChannel(channelId, serverId);
    } else {
      window._voiceRecorderUI = new VoiceRecorderUI({ channelId, serverId });
      const inputArea = document.querySelector('.message-input-area, .chat-input');
      if (inputArea) window._voiceRecorderUI.mount(inputArea);
    }
  });
});
