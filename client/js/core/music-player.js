...
function initMusicPlayer() {
  ...
  socket.on('music:play', ({ channelId, track }) => {
    if (!getCurrentChannel() || currentChannel._id !== channelId) return;
...
async function openMusicQueue() {
  if (!currentServer || !currentChannel) return toast('Önce bir kanala gir', 'error');
    ...
    modal.innerHTML = `
      <div class="modal-card" style="max-width:480px;width:95%;">
        ...
        <div style="display:flex;gap:8px;margin-bottom:16px;">
          ...
          <button class="btn btn-primary" onclick="musicAddToQueue()">▶ Ekle</button>
        </div>
        <div id="music-queue-list" style="max-height:300px;overflow-y:auto;"></div>
        <div style="margin-top:12px;display:flex;gap:8px;">
          <button class="btn" style="flex:1;" onclick="musicSkip()">⏭ Sonraki</button>
          <button class="btn btn-danger" style="flex:1;" onclick="musicStop()">⏹ Durdur</button>
        </div>
