...
//       Channels sekmesine bildirim noktasÄ±
...
//   Sesli mesaj transkripsiyonu hazÄ±r olunca gÃ¼ncelle
  socket.on('message:transcript', ({ messageId, transcript }) => {
    ...
    if (vtEl) {
      ...
      vtEl.innerHTML = `<span class="vt-icon">ğŸ“</span><span class="vt-text">${escHtml(transcript)}</span>`;
    }
  });

//   Link Ã¶nizleme embed gÃ¼ncellemesi
...
//     aktif ses kanalÄ±nÄ±n bitrate'i deÄŸiÅŸtiyse hemen uygula
...
//   Bot modal gÃ¶sterme
  ...
  socket.on('mention:received', ({ fromUser, preview }) => {
    ...
    const orig = document.title; document.title = `ğŸ”” Mention! â€” Bridge`; setTimeout(() => { document.title = orig; }, 4000);
  });
...
//   Admin sistem duyurusu
  socket.on('system_announcement', ({ message, from, ts }) => {
    ...
    el.innerHTML = `ğŸ“¢ <strong>Sistem Duyurusu</strong><br><span style="font-weight:400;">${escHtml(message)}</span>`;
  ...
  socket.on('connect_error', (e) => {
    ...
    if (e.message === 'IP banned') {
      ...
      const msg = mins
        ? `ğŸš« IP adresiniz ${mins} dakika engellendi: ${d.reason || ''}`
        : `ğŸš« IP adresiniz engellendi: ${d.reason || ''}`;
      toast(msg, 'error');
    } else if (e.message === 'Too many connections') {
      const d = e.data || {};
      toast(`â±ï¸ Ã‡ok fazla baÄŸlantÄ± giriÅŸimi. ${d.retryAfter || 60}sn sonra tekrar deneyin.`, 'error');
    } else {
      toast('Connection error. Reconnecting...', 'error');
    }
  });
  socket.on('reconnect', async () => {
    toast('ğŸŸ¢ Reconnected!', 'success');
    if (currentServer) loadMembers(currentServer._id);

    // â”€â”€ Offline mesaj kuyruÄŸunu flush et (SW outbox) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ KaÃ§Ä±rÄ±lan mesajlarÄ± senkronize et â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // Reconnect Ã¶ncesi gÃ¶nderilen mesajlar socket Ã¼zerinden gelmemiÅŸ
    // olabilir. Aktif kanalÄ± yeniden yÃ¼kle.
    if (currentChannel?._id && typeof loadMessages === 'function') {
      try {
        await loadMessages(currentChannel._id);
      } catch { /* sessizce geÃ§ â€” zaten baÄŸlantÄ± yeni kuruldu */ }
    }

    // â”€â”€ Bekleyen in-memory kuyruk mesajlarÄ±nÄ± gÃ¶nder â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    _flushPendingQueue();
  });
  socket.on('dm:message', (msg) => {
    if (currentDm && msg.dmId === currentDm._id) {
      ...
      if (msg.e2e && typeof (window as any).decryptIncoming === 'function') {
        (window as any).decryptIncoming(msg, el);
      }
    } else if (msg.userId !== (window as any).me?.id) {
...
// â”€â”€ v38: Bot Modal Renderer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  overlay.innerHTML = `
    <div class="modal-card" style="max-width:440px;width:95%;border-radius:12px">
      ...
      <div class="modal-footer" style="margin-top:16px;display:flex;gap:8px">
        <button class="btn btn-primary" id="_bm-submit">GÃ¶nder</button>
        <button class="btn" onclick="document.getElementById('_bot-modal')?.remove()">Ä°ptal</button>
      </div>
    </div>`;
