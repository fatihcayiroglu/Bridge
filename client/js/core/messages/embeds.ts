...
function renderEmbed(embed: any): string {
  if (embed.type === 'link') {
  ...
  const fieldsHtml = (embed.fields || []).length
    ? `<div class="embed-fields">${embed.fields.map((f: any) =>
...
function renderComponents(components: any[], messageId: string): string {
    ...
    for (const comp of row.components) {
      if (comp.type === 2) {
        ...
        html += `<select class="component-select" onchange="handleSelectComponent('${messageId}','${escHtml(comp.custom_id||'')}',this)">
          ...
          ${(comp.options||[]).map((o: any) => `<option value="${escHtml(o.value)}">${o.emoji ? o.emoji.name + ' ' : ''}${escHtml(o.label)}</option>`).join('')}
        </select>`;
      }
    }
    html += '</div>';
...
async function handleComponentClick(messageId: string, customId: string, e: any): Promise<void> {
...
async function handleSelectComponent(messageId: string, customId: string, selectEl: any): Promise<void> {
