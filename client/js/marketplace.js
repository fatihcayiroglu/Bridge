// client/js/marketplace.js — Bot Marketplace UI
(function () {
    ...
    overlay.innerHTML = `
      <div class="modal-card" style="max-width:800px;width:96%;max-height:88vh;overflow:hidden;display:flex;flex-direction:column;padding:0">
        <div style="padding:20px 24px 12px;border-bottom:1px solid var(--bg-4);display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap">
          ...
          <button onclick="document.getElementById('marketplace-modal').remove()" style="background:none;border:none;cursor:pointer;font-size:22px;color:var(--text-3);padding:4px 8px">✕</button>
        </div>
        <div style="padding:12px 24px;border-bottom:1px solid var(--bg-4);display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          <input id="mkt-search" type="text" class="input-field" placeholder="🔍 Bot ara..." style="flex:1;min-width:160px;padding:8px 12px;font-size:13px" oninput="debounceMktSearch(this.value)">
        </div>
  ...
  async function loadMarketplaceBots(q = '') {
    ...
    for (const bot of bots) {
      ...
      const stars = '⭐'.repeat(Math.round(bot.rating || 0));
      card.innerHTML = `
        <div class="bot-card-header">
          <div class="bot-avatar">${bot.icon ? `<img src="${escHtml(bot.icon)}" width="44" height="44" style="border-radius:50%">` : '🤖'}</div>
          <div>
            <div class="bot-name">${escHtml(bot.username)} ${bot.verified ? '<span class="bot-verified-badge">✓ Doğrulandı</span>' : ''}</div>
            <div class="bot-category">${escHtml(bot.category)}</div>
          </div>
        </div>
        <div class="bot-desc">${escHtml(bot.description || 'Açıklama yok.')}</div>
        <div class="bot-stats">
          <span title="Sunucu sayısı">🌐 ${bot.serverCount} sunucu</span>
          <span title="Komut sayısı">⚡ ${bot.commands} komut</span>
  ...
  window.addBotToServer = async function (botId, botName) {
    ...
    toast(`${botName} sunucuya eklendi! 🤖`, 'success');
  };
})();

// ── Marketplace açma butonu için filtre chip CSS ──
...
export const marketplaceReady = true;
