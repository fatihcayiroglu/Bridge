...
//
// Sprint 33 FIX: BridgeRegistry artık import ile alınıyor — global window'a bağımlılık yok.
...
(function () {
  ...
  // ── OPEN THREAD ──────────────────────────────────────────────
  BridgeRegistry.register('openThread', async function openThread(messageId: unknown, previewText: unknown) {
    // Zaten aynı thread açıksa kapat
    if (currentThread && currentThread.parentMessageId === messageId) {
      return BridgeRegistry.call('closeThread');
    }
  ...
  BridgeRegistry.register('closeThread', function closeThread() {
    ...
    panel.innerHTML = `
      <div class="thread-header">
        ...
        <div style="flex:1;min-width:0;">
          ...
          ${parentPreview ? `<div class="thread-parent-preview" style="font-size:11px;color:var(--text-muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:200px;" title="${escHtml(parentPreview)}">â†© ${escHtml(parentPreview.slice(0,60))}${parentPreview.length>60?'…':''}</div>` : ''}
        </div>
...
//     çeviri + tepki butonları
    const canEdit = msg.userId === (BridgeRegistry.call('getCurrentUserId'));
    const actionBtns = `
  ...
  // ── SEND MESSAGE ─────────────────────────────────────────────
  BridgeRegistry.register('sendThreadMessage', async function sendThreadMessage() {
    ...
    const optimisticMsg = {
      _id: `opt-${Date.now()}`,
      userId: BridgeRegistry.call('getCurrentUserId'),
      displayName: BridgeRegistry.call<{ displayName?: string }>('getCurrentUser')?.displayName || 'Sen',
      avatarColor: BridgeRegistry.call<{ avatarColor?: string }>('getCurrentUser')?.avatarColor || '#2d9cdb',
      content,
      createdAt: Date.now(),
    };
...
//   thread badge'ini ana kanalda güncelle/oluştur
  function _ensureThreadBadge(parentMsgId, thread) {
    const channelId = currentThread?.channelId || BridgeRegistry.call<{ _id?: string }>('getCurrentChannel')?._id;
    ...
    if (!badge) {
      ...
      if (msgEl) {
        ...
        if (newBadge) {
          ...
          newBadge.textContent = `ğŸ§µ ${thread.messageCount || 0} yanıt`;
          newBadge.onclick = () => BridgeRegistry.call('openThread', parentMsgId, thread.name);
        }
      }
    }
  }
  ...
  // ── THREAD İÇİ ÇEVİRİ ───────────────────────────────────────
  BridgeRegistry.register('translateThreadMessage', async function translateThreadMessage(msgId: string, btn: HTMLElement) {
    ...
    try {
      ...
      if (r.ok) {
        const d = await r.json();
        trEl.innerHTML = `<span style="color:var(--text-muted);font-size:11px;">🌐 ${escHtml(d.targetLang || '')} Â· </span><span style="color:var(--text-normal);">${escHtml(d.translated)}</span>`;
  ...
  // ── Thread mesaj sil ─────────────────────────────────────────
  BridgeRegistry.register('deleteThreadMessage', async function deleteThreadMessage(msgId: unknown) {
  ...
  BridgeRegistry.register('handleThreadKey', function handleThreadKey(e: KeyboardEvent) {
    if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendThreadMessage(); }
  });

  // ── SOCKET REAL-TIME ─────────────────────────────────────────
  BridgeRegistry.register('_bindThreadSocketEvents', function bindThreadSocketEvents() {
    socket.on('thread:message:new', ({ threadId, msg }) => {
      ...
      if (badge) {
        ...
        badge.textContent = `ğŸ§µ ${n} yanıt`;
      }
    });
  });
})();
