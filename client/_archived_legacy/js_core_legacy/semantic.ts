...
//
// Yerine geçen: client/js/core/SemanticPanel.svelte
//              client/js/core/semantic-svelte.ts (mount shim)
//
// BridgeRegistry kayıtları shim dosyasında korunuyor.
// Silinme planı: Sprint 118
//

// client/js/core/semantic.ts
// AI Akıllı Arama, Haftalık Digest, Bağlılık Dashboard

import { getMe, currentDmUserId } from './globals';
  ...
  if (chEl) {
    ...
    setTimeout(() => {
      ...
      if (el) {
        ...
        el.style.background = 'var(--brand-20,rgba(45,156,219,.2))';
...
async function loadDigest() {
  ...
  try {
    ...
    cont.innerHTML = `
      <!-- AI Özet -->
      ${d.aiSummary ? `
      <div style="background:linear-gradient(135deg,var(--brand-dark,#3c4491),var(--brand,#2d9cdb));border-radius:10px;padding:14px 16px;margin-bottom:16px;color:#fff;">
...
async function toggleDmE2E() {
  const _e2e = BridgeRegistry.get('BridgeE2E') as { getStatus?(): { enabled: boolean }; setup(id: string): Promise<void>; encryptDM(t: string, r: string, m: string): Promise<unknown> } | null;
  if (!_e2e) return toast('E2EE modülü yüklü değil', 'error');

  const status = _e2e.getStatus?.();
  if (!status?.enabled) {
    // E2EE kurulmamış — kur
  ...
  if (_dmE2EActive) {
    ...
    if (currentDmUserId) {
      try {
        const r = await apiFetch(`${API}/api/e2e/keys/${currentDmUserId}`);
        const d = await r.json();
...
// window-ref: monkey-patch bridge (unavoidable — cross-module compat)
const _origSendDm = BridgeRegistry.get('sendDm') ?? (async () => {});
BridgeRegistry.register('sendDm', async function() {
  if (!_dmE2EActive || !BridgeRegistry.get('BridgeE2E') || !currentDmUserId) {
  ...
  try {
    ...
    const __e2eDm = BridgeRegistry.get('BridgeE2E') as { encryptDM(t: string, r: string, m: string): Promise<unknown> } | null;
    const encrypted = await __e2eDm?.encryptDM(input.value, currentDmUserId!, myId);
    if (!encrypted) {
