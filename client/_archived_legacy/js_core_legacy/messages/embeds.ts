...
//
// Yerine geçen: client/js/core/EmbedsPanel.svelte
//              client/js/core/embeds-svelte.ts (mount shim)
//
// BridgeRegistry kayıtları shim dosyasında korunuyor.
// Silinme planı: Sprint 118
//
// core/messages/embeds.ts
// ── Embed & Component tipler ─────────────────────────────────
interface EmbedField { name: string; value: string; inline?: boolean; }
interface EmbedData {
  title?: string; description?: string; url?: string; color?: string | number;
  thumbnail?: { url: string }; image?: { url: string }; author?: { name: string; url?: string };
  footer?: { text: string }; fields?: EmbedField[]; timestamp?: string;
}
interface ComponentOption { value: string; label: string; emoji?: { name: string }; }
interface Component { type: number; style?: number; label?: string; custom_id?: string; options?: ComponentOption[]; components?: Component[]; }


// Embed kartları + bot component render (buttons, select)

function renderEmbed(embed: EmbedData): string {
  // Sprint 60: Spotify iframe embed
  if (embed.type === 'spotify' && (embed as Record<string, unknown>).embedSrc) {
    const src    = String((embed as Record<string, unknown>).embedSrc);
    const height = Number((embed as Record<string, unknown>).embedHeight) || 80;
    return `<div class="message-embed spotify-embed" style="border-left-color:#1DB954;padding:0;overflow:hidden;border-radius:0 8px 8px 0;max-width:480px;background:transparent;margin-top:4px">
      <iframe
        src="${escHtml(src)}"
        width="100%"
        height="${height}"
        ...
        loading="lazy"
        style="border-radius:0 8px 8px 0;display:block"
  ...
  const fieldsHtml = (embed.fields || []).length
    ? `<div class="embed-fields">${embed.fields.map((f: EmbedField) =>
...
function renderComponents(components: Component[], messageId: string): string {
  const styleMap: Record<number, string> = { 1: 'btn-brand', 2: 'btn-secondary', 3: 'btn-success', 4: 'btn-danger', 5: 'btn-link' };
  ...
  for (const row of components) {
    ...
    for (const comp of row.components) {
      if (comp.type === 2) {
        const cls = styleMap[comp.style as number] || 'btn-secondary';
        if (comp.style === 5) {
        ...
        html += `<select class="component-select" onchange="handleSelectComponent('${messageId}','${escHtml(comp.custom_id||'')}',this)">
          ...
          ${(comp.options||[]).map((o: ComponentOption) => `<option value="${escHtml(o.value)}">${o.emoji ? o.emoji.name + ' ' : ''}${escHtml(o.label)}</option>`).join('')}
        </select>`;
      }
    }
    html += '</div>';
...
async function handleComponentClick(messageId: string, customId: string, e: MouseEvent): Promise<void> {
...
async function handleSelectComponent(messageId: string, customId: string, selectEl: HTMLSelectElement): Promise<void> {
