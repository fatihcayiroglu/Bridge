...
//
// Yerine geçen: client/js/core/SettingsPanel.svelte
//              client/js/core/settings-svelte.ts (mount shim)
//
...
async function loadServerGifs() {
  ...
  container.innerHTML = '<div class="gif-loading">Yükleniyor...</div>';
  try {
    const r = await apiFetch(`${API}/api/servers/${currentServer._id}/gifs`);
    const gifs = await r.json();
    if (!gifs.length) { container.innerHTML = '<div class="gif-loading">Bu sunucuya henüz GIF eklenmemiş.</div>'; return; }
    container.innerHTML = '';
    for (const g of gifs) {
  ...
  } catch { container.innerHTML = '<div class="gif-loading">Yükleme başarısız.</div>'; }
}
...
async function uploadServerGif() {
  ...
  if (!name) return toast('GIF adı gerekli', 'error');
...
let _scStatusCb: ((d: Record<string, unknown>) => void) | null = null;
let _scLogCb:    ((d: Record<string, unknown>) => void) | null = null;

async function openServerControlPanel() {
  const sc = (window as Record<string, unknown>).serverControl;
  if (!sc) {
  ...
  _scStatusCb = (d: Record<string, unknown>) => scUpdateUI(d.status, d.pid);
  _scLogCb    = (d: Record<string, unknown>) => scAppendLog(d);

  sc.onStatus(_scStatusCb);
  sc.onLog(_scLogCb);
}

function scStart()   { (window as Record<string, unknown>).serverControl?.start(); }
function scStop()    { (window as Record<string, unknown>).serverControl?.stop(); }
function scRestart() { (window as Record<string, unknown>).serverControl?.restart(); }
...
document.addEventListener('DOMContentLoaded', () => {
  if (!(window as Record<string, unknown>).serverControl) return;
...
(window as Record<string, unknown>).openServerControlPanel = openServerControlPanel;
(window as Record<string, unknown>).scStart   = scStart;
(window as Record<string, unknown>).scStop    = scStop;
(window as Record<string, unknown>).scRestart = scRestart;
(window as Record<string, unknown>).scClearLogs = scClearLogs;
