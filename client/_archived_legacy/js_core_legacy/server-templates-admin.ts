...
//
// Yerine geçen: client/js/core/ServerTemplatesAdminPanel.svelte
//              client/js/core/server-templates-admin-svelte.ts (mount shim)
//
// BridgeRegistry kayıtları shim dosyasında korunuyor.
// Silinme planı: Sprint 118
//
// core/server-templates-admin.js
// Sunucu Åablonları Yönetim Arayüzü
...
/** Åablon yönetim modalını açar ve listeyi yükler */
...
async function _sttLoadList() {
  ...
  try {
    ...
    listEl.innerHTML = `<div class="stt-error">âš ï¸ Åablonlar yüklenemedi: ${escHtml(err.message)}</div>`;
  }
}
...
function sttOpenEdit(id) {
  ...
  if (!tpl) return toast('Åablon bulunamadı', 'error');
  _sttEditId = id;
  _sttShowFormModal({
    name:        tpl.name        || '',
    icon:        tpl.icon        || 'ğŸ“‹',
...
function _sttShowFormModal(data) {
  ...
  const title = _sttEditId ? 'Åablonu Düzenle' : 'Yeni Åablon';
  modal.innerHTML = `
    <div class="modal" style="max-width:480px;width:92%">
      ...
      <div class="modal-body" style="padding:20px;display:flex;flex-direction:column;gap:14px">
        ...
        <label class="stt-label">
          <span>Åablon Adı <span style="color:var(--danger)">*</span></span>
...
async function sttSaveForm() {
  ...
  if (!name) {
    errEl.textContent = 'Åablon adı zorunludur.';
    errEl.style.display = '';
    return;
  }
  ...
  try {
    ...
    toast(isEdit ? 'Åablon güncellendi' : 'Åablon oluşturuldu', 'success');
    sttCloseFormModal();
    await _sttLoadList();
  } catch (err) {
...
async function sttOpenDetail(id) {
  ...
  modal.innerHTML = `
    <div class="modal" style="max-width:520px;width:92%;max-height:85vh">
      <div class="modal-header">
        <h3 style="margin:0;font-size:16px">Åablon Önizleme</h3>
  ...
  try {
    ...
    document.getElementById('stt-detail-body')?.innerHTML =
      `<div class="stt-error">âš ï¸ Detay yüklenemedi: ${escHtml(err.message)}</div>`;
  }
}
...
function _injectTemplatesAdminModal() {
  ...
  modal.innerHTML = `
    <div class="modal" style="max-width:680px;width:96%;max-height:90vh;display:flex;flex-direction:column">
      <div class="modal-header">
        <h3 style="margin:0;font-size:16px">ğŸ“‹ Sunucu Åablonları</h3>
...
(function _injectSttStyles() {
  ...
  s.textContent = `
    ...
    .stt-card:hover { border-color: var(--brand-light, rgba(45,156,219,.35)); }
