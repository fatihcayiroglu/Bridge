...
//
// Yerine geçen: client/js/core/SearchPanel.svelte
//              client/js/core/search-svelte.ts (mount shim)
//
// BridgeRegistry kayıtları shim dosyasında korunuyor.
// Silinme planı: Sprint 118
//
// client/js/core/search.ts
...
let gsFromMap: Record<string, string> = {};          // userId → displayName (chip label için)
let gsPage    = 0;
...
function onGlobalSearch(val) {
  ...
  if (!val.trim()) {
    ...
    if (res) res.innerHTML = '<div class="gs-empty">Aramak için yazmaya başlayın…<br><span class="gs-hint">İpucu: <code>from:ali</code> Â· <code>has:file</code> Â· <code>before:2025-06-01</code></span></div>';
...
function gsRenderChips() {
  ...
  if (gsFilters.after) chips.push({ label: `ğŸ“… ${gsFilters.after} sonrası`,           key: 'after' });
  if (gsFilters.before)chips.push({ label: `ğŸ“… ${gsFilters.before} öncesi`,           key: 'before'});
...
function renderGsResults(data, q) {
  ...
  // ── Üyeler ───────────────────────────────────────────────────
  if (members.length) {
    html += `<div class="gs-section-label">ğŸ‘¥ Üyeler (${members.length})</div>`;
    html += members.map(m => `
      <div class="gs-item" onclick="openDmWithUser('${m._id}','${escHtml(m.displayName)}','${m.avatarColor || '#4f8ef7'}');closeGlobalSearch()">
        ...
        <button class="gs-action-btn" onclick="event.stopPropagation();gsFilterFrom('${m._id}','${escHtml(m.displayName)}')" title="Bu kişiden mesajları ara">ğŸ”</button>
      </div>`).join('');
  }
    ...
    html += msgs.map(m => {
      ...
      return `
        <div class="gs-item gs-msg-item" onclick="jumpToMessage('${m._id}','${m.channelId}','${currentServer._id}');closeGlobalSearch()">
          ...
          <button class="gs-action-btn" onclick="event.stopPropagation();copyToClipboard('${escHtml(m.content?.slice(0,200)||'')}');" title="İçeriği kopyala">ğŸ“‹</button>
        </div>`;
    }).join('');
  }
...
async function openPinsPanel() {
  ...
  panel.innerHTML = `
    <div class="pins-header">
      <span>ğŸ“Œ Sabitlenmiş Mesajlar</span>
