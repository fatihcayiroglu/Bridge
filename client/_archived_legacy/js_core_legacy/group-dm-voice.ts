...
//
// Yerine geçen: client/js/core/GroupDmVoicePanel.svelte
//              client/js/core/group-dm-voice-svelte.ts (mount shim)
//
...
// ── Module-level state variables (declared in group-dm-core.js at runtime) ──
declare var _gdmGroups:      Array<{ _id: string; name?: string }>;
declare var _gdmCallGroupId: string | null;
declare var _gdmCallType:    string;
declare var _gdmLocalStream: MediaStream | null;
const _gdmCallPeers: Map<string, GdmPeer> = new Map();
...
function _renderCallBar() {
  ...
  bar.innerHTML = `
    ...
    <span style="font-weight:600;font-size:13px">${escHtml(g.name || 'Grup Araması')} (${peerCount + 1})</span>
    ...
    <button id="gdm-call-expand-btn" class="btn btn-sm" title="Büyüt" style="font-size:16px" onclick="openGdmCallModal()">â›¶</button>
...
function _renderCallGrid() {
  ...
  const localTile = _makePeerTile({
    ...
    avatarColor: me?.avatarColor || '#2d9cdb',
  ...
  popup.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px">
      ...
      <div>
        ...
        <div style="font-size:12px;color:var(--text-muted)">${icon} ${escHtml(g.name)} Â· ${type === 'video' ? 'Görüntülü' : 'Sesli'} arama</div>
      </div>
    </div>
    <div style="display:flex;gap:8px">
      <button class="btn btn-sm" style="flex:1;background:var(--green,#57f287);color:#000;font-weight:700" onclick="joinGdmCall('${groupId}','${type}');document.getElementById('gdm-incoming-call')?.remove()">ğŸ“ Yanıtla</button>
...
function bindGroupDmSocketEvents(socket) {
    ...
    for (const peer of peers) {
      if (!_gdmCallPeers.has(peer.socketId)) {
        _gdmCallPeers.set(peer.socketId, {
          ...
          avatarColor: peer.avatarColor || '#2d9cdb',
          stream:      null,
          pc:          null,
          muted:       false,
        });
  ...
  socket.on('gdm:call:peer:joined', async ({ groupId, userId, displayName, avatarColor, socketId }) => {
    ...
    if (!_gdmCallPeers.has(socketId)) {
      _gdmCallPeers.set(socketId, {
        ...
        avatarColor: avatarColor || '#2d9cdb',
    ...
    _renderCallPeersBar();
    _renderCallGrid();
    toast(`${escHtml(displayName)} aramaya katıldı ğŸ™ï¸`, 'info');
  });
  ...
  socket.on('gdm:call:offer', async ({ groupId, fromSocketId, offer }) => {
    ...
    if (!peer) {
      ...
      peer = { userId: null, displayName: 'Kullanıcı', avatarColor: '#2d9cdb', stream: null, pc: null, muted: false };
    ...
    try {
      ...
      log.error('[GDM Voice] offer işleme hatası:', err);
    }
  });
  ...
  socket.on('gdm:call:answer', async ({ groupId, fromSocketId, answer }) => {
    ...
    try {
      ...
      log.error('[GDM Voice] answer işleme hatası:', err);
    }
  });
  ...
  socket.on('gdm:call:ice', async ({ groupId, fromSocketId, candidate }) => {
    ...
    try {
      ...
      log.error('[GDM Voice] ICE candidate hatası:', err);
    }
  });
