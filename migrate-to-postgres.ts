...
const transforms = {
  ...
  // --- v65+ yeni tablolar ---
  native_push_tokens:  row => row,
  notifications:       row => ({ ...row, read: boolify(row.read) }),
  ap_outgoing_follows: row => ({ ...row, accepted: boolify(row.accepted) }),
  ap_likes:            row => row,
  ap_announces:        row => row,
  ap_delivery_queue:   row => ({ ...row, payload: parseJson(row.payload, {}) }),
  federation_whitelist: row => row,
  federation_blacklist: row => row,
  link_preview_cache:  row => ({ ...row, data: parseJson(row.data, {}) }),
  server_templates:    row => ({ ...row, config: parseJson(row.config, {}), isBuiltin: boolify(row.isBuiltin) }),
  podcast_settings:    row => ({ ...row, enabled: boolify(row.enabled) }),
  podcast_episodes:    row => row,
};
...
const MIGRATION_ORDER = [
  ...
  'native_push_tokens', 'notifications',
  // Log
  ...
  // Federation
  'federation_peers', 'federation_whitelist', 'federation_blacklist',
  'ap_follows', 'ap_outgoing_follows', 'ap_activities', 'ap_messages',
  'ap_likes', 'ap_announces', 'ap_delivery_queue',
  // Auth & güvenlik
  'refresh_tokens', 'webauthn_credentials',
  // Cache & şablonlar
  'link_preview_cache', 'server_templates',
  // Podcast
  'podcast_settings', 'podcast_episodes',
];
